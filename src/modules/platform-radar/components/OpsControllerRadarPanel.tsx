...
export function OpsControllerRadarPanel({ className }: { className?: string }) {
  ...
  return (
    <div
      className={cn(
        'rounded-2xl border border-sky-400/20 bg-black/72 px-[clamp(0.85rem,1.8vw,1.25rem)] py-[clamp(0.65rem,1.2vh,0.9rem)] backdrop-blur-xl',
        className,
      )}
    >
      ...
      <div className="mt-3 rounded-xl border border-white/10 bg-black/45 px-3 py-2.5">
        ...
        <ul className="m-0 list-none space-y-1.5 p-0" dir="rtl">
          {lines.map((row) => (
            ...
            >
              <span className={cn('font-bold tabular-nums', statusTone(row.status))}>
                {loading ? '…' : row.status}
              </span>
