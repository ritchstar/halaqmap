 ...
 *
 * زخرفة خلفية خفيفة أنيقة بطابع الشطرنج — نقشة رقعة شبه شفافة + قطع
 * يونيكود باهتة بزوايا الصفحة (نفس رموز ♔♛♜♝♞♟ المستخدمة فعلياً في رقعة
 * ساحة اللعب `ChessBoardView.tsx`، لا صور خارجية). خفيفة بما يكفي لتبقى
 * صفحات المدرسة (التسجيل/التأكيد/الدفع) واضحة القراءة وبنفس ثيم shadcn
 * الفاتح الحالي — زخرفة، لا تغيير في الثيم العام.
 *
...
export function ChessMotifBackground() {
  const patternId = useId();

  return (
    <div aria-hidden="true" className="pointer-events-none absolute inset-0 select-none overflow-hidden">
      {/* نقشة رقعة شطرنج خفيفة جداً على كامل الخلفية */}
      ...
      {/* قطع شطرنج زخرفية كبيرة باهتة بزوايا الصفحة */}
      {CORNER_PIECES.map((piece) => (
        <span key={piece.glyph} className={`${piece.className} leading-none`}>
          {piece.glyph}
        </span>
      ))}
    </div>
