...
export const BARBER_CONTACT_SHEET_SHELL =
  'fixed z-50 flex min-w-0 flex-col gap-0 border bg-background p-4 shadow-lg ' +
  'left-1/2 top-1/2 w-[min(calc(100vw-1rem),56rem)] max-w-[min(calc(100vw-1rem),56rem)] max-h-[min(92dvh,90vh)] ' +
  '-translate-x-1/2 -translate-y-1/2 overflow-x-hidden overflow-y-auto overscroll-contain ' +
...
export const BARBER_CONTACT_SHEET_BODY =
  'barber-contact-inner mt-4 min-w-0 max-w-full overflow-x-clip space-y-5 sm:space-y-6';
...
export function BarberContactSheetSection({
  ...
  return <section className={cn(BARBER_CONTACT_SHEET_SECTION, className)}>{children}</section>;
}
