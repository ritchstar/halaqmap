...
function LaneBadge({ slideId }: { slideId: string }) {
  const lane = laneForSlide(slideId);
  const label = GROWTH_PITCH_LANE_LABEL[lane];
...
function SlideBody({
  ...
  if (slide.kind === 'hero') {
    return (
      <SlideFrame slide={slide} accent={accent} layout={layout}>
...
export function GrowthPitchSlideDeck() {
  ...
  const layout = layoutForSlide(slide.id);
      ...
      <main className="relative z-10 flex flex-1 items-center justify-center overflow-y-auto px-3 py-6 sm:px-6 sm:py-8">
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="w-full"
          >
            <SlideBody slide={slide} accent={accent} layout={layout} />
          </motion.div>
        </AnimatePresence>
      </main>
