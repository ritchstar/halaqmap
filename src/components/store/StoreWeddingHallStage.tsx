...
export function StoreWeddingHallStage({
  ...
  return (
    <div className={cn('relative overflow-hidden rounded-[28px] border border-[#d4af67]/35 bg-black text-[#f7edd8]', className)}>
      ...
      {state.host.announcement.trim() ? (
        <div className="absolute inset-x-4 top-4 z-20 rounded-2xl border border-[#d4af67] bg-black/75 px-4 py-3 text-center shadow-[0_0_40px_rgba(212,175,103,0.25)]">
          <p className="text-sm font-black tracking-wide text-[#d4af67] md:text-lg">{state.host.announcement.trim()}</p>
        </div>
      ) : null}
      ...
      <div className="relative z-10 flex min-h-[32rem] flex-col p-5 pt-16 md:p-8">
            ...
            {latest.length ? (
              latest.map((item) => (
                <li key={item.id} className="rounded-2xl border border-white/12 bg-black/50 p-4">
                  <p className="text-sm font-extrabold text-[#d4af67]">{item.name}</p>
        ...
        <div className="mt-auto overflow-hidden border-t border-[#d4af67]/30 pt-4">
