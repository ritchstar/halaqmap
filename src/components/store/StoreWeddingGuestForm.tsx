...
import { STORE_WEDDING_LIVE, STORE_WEDDING_LIVE_CANNED } from '@/config/storeWeddingLive';
...
export function StoreWeddingGuestForm({
  ...
  return (
    ...
    >
      <h2 className="text-lg font-extrabold">{STORE_WEDDING_LIVE.guestFormTitleAr}</h2>
      <p className="mt-1 text-xs text-white/50">{STORE_WEDDING_LIVE.guestOnlyHintAr}</p>
      <label className="mt-4 block text-sm">
        {STORE_WEDDING_LIVE.guestNameLabelAr}
        <input
          value={guestName}
      ...
      <label className="mt-4 block text-sm">
        {STORE_WEDDING_LIVE.guestExtraLabelAr}
        <textarea
          value={extra}
      ...
      <button type="submit" className="mt-4 w-full rounded-full bg-[#e8c547] py-2.5 text-sm font-bold text-[#061018]">
        {STORE_WEDDING_LIVE.guestSubmitAr}
      </button>
      {sent ? <p className="mt-3 text-sm text-[#e8c547]">ظهرت تهنئتك على شاشة القاعة.</p> : null}
    </form>
  );
}
