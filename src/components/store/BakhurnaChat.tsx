 ...
 *
 * صندوق ملاحظة بخورنا1 — مدرج، ليست غرفة عامة.
...
import type { BakhurnaChatMsg, BakhurnaLabState } from '@/lib/storeBakhurnaLiveLab';
...
const themeClasses = {
  dark: {
    section: 'border-[#6E4A26]/25 bg-[#1a140c]/80',
    hint: 'text-white/60',
    field: 'bakhurna-field',
    button: 'bg-[#6E4A26] text-[#f7edd8]',
...
export function StoreBakhurnaBuyerChat({
  state,
  onChange,
  theme = 'dark',
}: {
  ...
  const fieldClass = theme === 'light' ? t.field : 'bakhurna-field';

  function send() {
    const body = text.trim();
    if (body.length < 2) return;
    const msg: BakhurnaChatMsg = {
      id: `${Date.now()}`,
      from: 'buyer',
  ...
  return (
    <section className={cn('mt-6 rounded-2xl border p-4', t.section)}>
      <h3 className={cn('font-extrabold', t.heading)}>{STORE_BAKHURNA_LIVE.chatBuyerTitleAr}</h3>
      <p className={cn('mt-1 text-xs leading-6', t.hint)}>{STORE_BAKHURNA_LIVE.chatBuyerHintAr}</p>
      <label className={cn('mt-3 block text-sm', t.body)}>
        {STORE_BAKHURNA_LIVE.chatBuyerNameLabelAr}
        <input className={fieldClass} value={name} onChange={(e) => setName(e.target.value)} />
      </label>
      <label className={cn('mt-3 block text-sm', t.body)}>
        {STORE_BAKHURNA_LIVE.chatBuyerFieldLabelAr}
        <textarea
      ...
      <button type="button" onClick={send} className={cn('mt-3 rounded-full px-4 py-2 text-sm font-bold', t.button)}>
        {STORE_BAKHURNA_LIVE.chatBuyerSendAr}
      </button>
    </section>
  );
}

export function StoreBakhurnaDeskChat({
  state,
  onChange,
  theme = 'dark',
}: {
  ...
  function sendReply() {
    ...
    const msg: BakhurnaChatMsg = {
      id: `${Date.now()}`,
      from: 'desk',
      name: state.host.hostName || 'المحل',
      text: body.slice(0, 240),
      at: new Date().toISOString(),
    };
    onChange({ ...state, chats: [msg, ...state.chats].slice(0, 200) });
  ...
  return (
    <section className={cn('rounded-2xl border p-4', t.section)}>
      <h3 className={cn('font-extrabold', t.heading)}>{STORE_BAKHURNA_LIVE.chatDeskTitleAr}</h3>
      ...
      <label className={cn('mt-3 block text-sm', t.body)}>
        {STORE_BAKHURNA_LIVE.chatDeskReplyAr}
