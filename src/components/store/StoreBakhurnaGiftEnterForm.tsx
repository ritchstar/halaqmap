...
import {
  ...
  STORE_BAKHURNA_GIFT_SOURCE_OPTIONS,
  STORE_BAKHURNA_GIFT_TERMS_COPY,
  type StoreBakhurnaGiftSource,
} from '@/config/storeBakhurnaGiftCampaign';
import { enterStoreBakhurnaGift } from '@/lib/storeBakhurnaGiftCampaignRemote';
import { ROUTE_PATHS } from '@/lib/routePaths';
import { cn } from '@/lib/utils';

const fieldClass =
  'mt-1 w-full rounded-xl border border-white/15 bg-[#061018] px-3 py-2.5 text-sm text-[#f4efe4] outline-none focus:border-[#6E4A26]/50';

export function StoreBakhurnaGiftEnterForm({ accepting }: { accepting: boolean }) {
  const copy = STORE_BAKHURNA_GIFT_COPY;
  const [givenName, setGivenName] = useState('');
  const [city, setCity] = useState('');
  const [source, setSource] = useState<StoreBakhurnaGiftSource | ''>('');
  ...
  async function onSubmit(event: FormEvent) {
    ...
    setBusy(true);
    const result = await enterStoreBakhurnaGift({
      givenName,
      email,
      city,
      source,
  ...
  return (
    <form
      id="store-bakhurna-gift-enter"
      onSubmit={onSubmit}
      className="relative rounded-2xl border border-[#6E4A26]/30 bg-[#1a140c]/80 p-5 md:p-6"
    >
      ...
      <label className="mt-4 block text-sm font-bold text-white/80">
        {copy.sourceLabelAr}
        <select className={fieldClass} value={source} onChange={(e) => setSource(e.target.value as StoreBakhurnaGiftSource | '')}>
          <option value="">—</option>
          {STORE_BAKHURNA_GIFT_SOURCE_OPTIONS.map((item) => (
      ...
      <Collapsible open={termsOpen} onOpenChange={setTermsOpen} className="mt-5">
        <CollapsibleTrigger
          type="button"
          className="flex w-full items-center justify-between gap-3 rounded-xl border border-[#6E4A26]/25 bg-white/5 px-4 py-3 text-right text-sm font-bold text-[#6E4A26]"
        >
        ...
        <CollapsibleContent className="mt-3 space-y-3 rounded-xl border border-white/10 bg-[#061018]/70 p-4">
          {STORE_BAKHURNA_GIFT_TERMS_COPY.sections.map((section) => (
          ...
          <Link to={ROUTE_PATHS.STORE_BAKHURNA_GIFT_TERMS} className="inline-flex text-sm font-bold text-[#6E4A26]">
            {copy.termsLinkAr}
          </Link>
        </CollapsibleContent>
      </Collapsible>
      ...
      <label className="mt-4 flex items-start gap-3 text-sm leading-7 text-white/80">
        <input
          type="checkbox"
          className="mt-1 h-4 w-4 accent-[#6E4A26]"
          checked={acceptedTerms}
          onChange={(e) => setAcceptedTerms(e.target.checked)}
        />
        <span>
          {copy.consentAr}{' '}
          <Link to={ROUTE_PATHS.STORE_BAKHURNA_GIFT_TERMS} className="font-bold text-[#6E4A26]">
            {copy.termsLinkAr}
          </Link>
        </span>
      </label>
      ...
      <button
        type="submit"
        disabled={busy}
        className="mt-5 w-full rounded-full bg-[#6E4A26] px-5 py-3 text-sm font-extrabold text-[#061018] disabled:opacity-60"
      >
        {copy.submitAr}
      </button>
    </form>
