 ...
 *
 * واجهة زبون بخورنا1 — هيكل Chatly مع منطق halaqmap الحقيقي.
...
import { StoreBakhurnaBuyerChat } from '@/components/store/BakhurnaChat';
...
import {
  isBakhurnaComeApproaching,
  parseMapsQueryCoords,
  requestBakhurnaComeNotify,
  showBakhurnaComeApproachingNotice,
} from '@/lib/storeBakhurnaCome';
import {
  bakhurnaCartTotal,
  readSavedBakhurnaBuyer,
  writeSavedBakhurnaBuyer,
  type BakhurnaLabState,
  type BakhurnaOrderLine,
  type BakhurnaPayMethod,
  type BakhurnaService,
...
export function BakhurnaChatlyStorefront({
  ...
  const saved = useMemo(() => readSavedBakhurnaBuyer(), []);
  ...
  const [pay, setPay] = useState<BakhurnaPayMethod>('cash');
  const [service, setService] = useState<BakhurnaService>('delivery');
  ...
  const lines: BakhurnaOrderLine[] = visible
    .map((item) => ({
  ...
  const total = bakhurnaCartTotal(lines);
  ...
  const categories = useMemo(() => {
    ...
    return ['الكل', ...(featured.length ? [STORE_BAKHURNA_LIVE.featuredTitleAr] : []), ...Array.from(cats).sort()];
  }, [shelfItems, featured.length]);

  const filtered = useMemo(() => {
    const q = search.trim();
    const base = category === STORE_BAKHURNA_LIVE.featuredTitleAr ? visible.filter((i) => i.featured) : shelfItems;
    return base.filter((item) => {
      const matchesCategory =
        category === 'الكل' ||
        (category === STORE_BAKHURNA_LIVE.featuredTitleAr ? item.featured : item.category === category);
  ...
  useEffect(() => {
    ...
    setComeHint(STORE_BAKHURNA_LIVE.comeApproachingAr);
    void showBakhurnaComeApproachingNotice(state.host.shopName);
  ...
  function priceLine(item: ShelfRow) {
    return `${item.price} ر.س / ${STORE_BAKHURNA_UNIT_AR[item.unit]}`;
  }
  ...
  async function submit() {
    ...
    const coords = come
      ...
      : null;
    if (come && !coords) {
      setComeHint(STORE_BAKHURNA_LIVE.comeNeedPlaceAr);
      return;
    }
    if (come) {
      const granted = await requestBakhurnaComeNotify();
      if (!granted) {
        setComeHint(STORE_BAKHURNA_LIVE.comeNotifyDeniedAr);
        return;
      }
    }
    const order = {
    ...
    writeSavedBakhurnaBuyer(saveBuyer ? { name: order.name, phone: order.phone, place: order.place } : null);
    setQty({});
    setSent(come ? '' : STORE_BAKHURNA_LIVE.orderSentAr);
    setOrderPlaced(true);
    setCartOpen(false);
    setCheckoutOpen(false);
    showNotice(come ? STORE_BAKHURNA_LIVE.comeWatchingAr : STORE_BAKHURNA_LIVE.orderSentAr);
    if (come) {
      setWatchingCome(true);
      setComeHint(STORE_BAKHURNA_LIVE.comeWatchingAr);
    }
  }
  ...
  const services = [
    { id: 'delivery' as const, label: STORE_BAKHURNA_LIVE.serviceDeliveryAr, icon: Truck, copy: 'إلى بابك داخل النطاق' },
  ...
  return (
    <main dir="rtl" className="bakhurna-chatly-root min-h-dvh bg-[#f3e6cf] pb-32 text-[#2a2016]">
      <div className="mx-auto w-full max-w-[1240px] px-4 sm:px-8 lg:px-10">
        <header className="flex items-center justify-between border-b border-[#e2d2b4] py-5">
          <div className="flex min-w-0 items-center gap-3">
            <div className="flex size-10 shrink-0 items-center justify-center rounded-full border-[#d8c19c] bg-[#fdf9f0] text-[#6e4a26]">
              <StoreShopLogoMark src={state.host.logoSrc} className="size-8" />
              {!state.host.logoSrc ? <BakhurnaMabkharaMark size="sm" /> : null}
            </div>
            <div className="min-w-0">
              <p className="text-[10px] font-bold tracking-[0.2em] text-[#8a6239]">{STORE_BAKHURNA_LIVE.shopKickerAr}</p>
              <p className="truncate text-lg font-black tracking-tight">{state.host.shopName}</p>
            </div>
            {mobile ? <StoreMobileVendorMark accent={STORE_BAKHURNA_LIVE_ACCENT} /> : null}
            <StoreShopPlacePin
              ...
              labelAr={STORE_BAKHURNA_LIVE.pickupPinAriaAr}
            />
          </div>
          <Button
            type="button"
            ...
            className="bakhurna-chatly-btn-ghost relative h-10 shrink-0 rounded-full px-4 text-xs font-black"
          >
            <ShoppingBag size={16} />
            السلة
            {cartLineCount ? (
              <span className="flex size-5 items-center justify-center rounded-full bg-[#6e4a26] text-[10px] text-white">
                {cartLineCount}
              </span>
            ) : null}
          </Button>
        ...
        {state.host.flashAr.trim() ? (
          <p className="mt-4 overflow-hidden rounded-2xl border border-[#d8c19c] bg-[#f3e6cf] px-4 py-2 text-sm font-bold text-[#6e4a26]">
            {state.host.flashAr}
          </p>
        ) : null}

        <StoreShopHoursBanner hours={state.host} accent={STORE_BAKHURNA_LIVE_ACCENT} />
        <StoreMobileVendorBanner place={state.host} closed={closed} accent={STORE_BAKHURNA_LIVE_ACCENT} />
        ...
        <section className="grid gap-8 border-b border-[#e2d2b4] py-9 lg:grid-cols-[1fr_0.86fr] lg:items-center lg:gap-16">
          <div>
            <div className="mb-4 flex items-center gap-2 text-xs font-bold text-[#8a6239]">
              ...
              {STORE_BAKHURNA_LIVE.shopKickerAr}
            </div>
            <h1 className="max-w-xl text-4xl font-black leading-[1.08] tracking-[-0.06em] text-[#2a2016] sm:text-5xl">
              {state.host.shopName}
              <br />
              <span className="text-[#8a6239]">{STORE_BAKHURNA_LIVE.heroCaptionAr}</span>
            </h1>
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <Button
                ...
                className="bakhurna-chatly-btn-primary h-12 rounded-full px-6 text-sm font-black shadow-[0_12px_25px_rgba(110,74,38,0.2)]"
              >
                اختَر طريقة الاستلام
                <ChevronDown size={16} />
              </Button>
              {arrived.length ? (
                <div className="inline-flex items-center gap-2 rounded-full border border-[#d8c19c] bg-[#fdf9f0] px-4 py-3 text-xs font-bold text-[#6f6250]">
                  <span className="size-2 rounded-full bg-[#6e4a26]" />
                  {STORE_BAKHURNA_LIVE.todayTitleAr}: {arrived[0].nameAr}
        ...
        {arrived.length ? (
          <section className="border-b border-[#e2d2b4] py-9">
            <p className="text-xs font-bold tracking-[0.16em] text-[#8a6239]">{STORE_BAKHURNA_LIVE.todayTitleAr}</p>
            <h2 className="mt-2 text-2xl font-black tracking-[-0.05em] text-[#2a2016]">{STORE_BAKHURNA_LIVE.todayTitleAr}</h2>
        ...
        {featured.length ? (
          <section className="border-b border-[#e2d2b4] py-9">
            <p className="text-xs font-bold tracking-[0.16em] text-[#8a6239]">{STORE_BAKHURNA_LIVE.featuredTitleAr}</p>
            <h2 className="mt-2 text-2xl font-black tracking-[-0.05em] text-[#2a2016]">{STORE_BAKHURNA_LIVE.featuredTitleAr}</h2>
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <section className="sticky top-0 z-20 -mx-4 border-b border-[#e2d2b4] bg-[#f3e6cf]/95 px-4 py-3 backdrop-blur-sm sm:-mx-8 sm:px-8 lg:-mx-10 lg:px-10">
          <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center">
            <div className="relative min-w-0 flex-1">
              ...
              <Input
                ...
                className="h-11 border-[#dac8aa] bg-[#fdf9f0] pr-10 text-sm text-[#2a2016] placeholder:text-[#a89a7e]"
              />
            </div>
              ...
              {categories.map((item) => (
                <Button
                  ...
                  className={cn(
                    ...
                    category === item ? 'bakhurna-chatly-btn-primary' : 'bakhurna-chatly-btn-ghost',
                  )}
                >
                  {item}
                </Button>
        ...
        <section id="shelf" className="py-9">
          <div className="flex items-end justify-between gap-3">
            <div>
              <p className="text-xs font-bold tracking-[0.16em] text-[#8a6239]">{STORE_BAKHURNA_LIVE.shelfTitleAr}</p>
              <h2 className="mt-2 text-3xl font-black tracking-[-0.05em] text-[#2a2016]">اختَر ما تحتاجه</h2>
              <p className="mt-2 text-sm text-[#79674f]">{STORE_BAKHURNA_LIVE.priceEstimateNoteAr}</p>
            </div>
          ...
          {!filtered.length ? (
            <div className="mt-6 rounded-2xl border border-dashed border-[#e2d2b4] bg-[#fdf9f0] p-10 text-center">
              <p className="mt-3 font-black text-[#2a2016]">ما لقينا هذا الصنف</p>
        ...
        <section className="grid gap-3 border-t border-[#e2d2b4] py-8 sm:grid-cols-3">
          <Feature icon={Truck} title={STORE_BAKHURNA_LIVE.serviceDeliveryAr} copy="توصيل واضح — الدفع عند الاستلام." />
          <Feature icon={MapPin} title={STORE_BAKHURNA_LIVE.servicePickupAr} copy="موقع المحل يظهر قبل التأكيد." />
          {mobile ? (
            <Feature icon={Navigation} title={STORE_BAKHURNA_LIVE.serviceComeAr} copy="تنبيه عند اقتراب العربة من موقعك." />
          ) : (
            <Feature icon={Store} title="متجر مستقل" copy="طلبك يذهب مباشرة إلى هذا المحل." />
          )}
        </section>

        <section className="rounded-2xl border border-[#dac8aa] bg-[#fdf9f0] p-5">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
            <div>
              <p className="text-xs font-bold tracking-[0.14em] text-[#8a6239]">متجر مستقل</p>
              <h3 className="mt-1 text-lg font-black text-[#2a2016]">أنت تشتري من بائع واحد في حيك</h3>
              <p className="mt-1 text-sm text-[#79674f]">ليست سوقاً مشتركة؛ طلبك يذهب مباشرة إلى {state.host.shopName}.</p>
            </div>
            <StoreDirectPayPublicMount product="store_bakhurna_live" token={token} accent={STORE_BAKHURNA_LIVE_ACCENT} />
          </div>
        </section>

        <div className="mt-6">
          <StoreBakhurnaBuyerChat state={state} onChange={onChange} theme="light" />
        </div>
        ...
        <footer className="flex flex-col gap-3 border-t border-[#e2d2b4] py-7 text-sm text-[#79674f] sm:flex-row sm:items-center sm:justify-between">
          <p>بخورنا1 — من المحل إلى جوال الحي.</p>
      ...
      {canCheckout ? (
        <div className="fixed bottom-4 right-4 left-4 z-30 mx-auto flex max-w-[800px] items-center justify-between gap-4 rounded-2xl bg-[#2a2016] px-4 py-3 text-white shadow-[0_15px_35px_rgba(42,32,22,0.28)] sm:right-8 sm:left-8 sm:px-5">
          <div className="flex min-w-0 items-center gap-3">
            <span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-[#6e4a26]">
              <BakhurnaMabkharaMark size="sm" inverse />
            </span>
            <div className="min-w-0">
              <p className="truncate text-sm font-black">
                {come && !cartItemCount
                  ? STORE_BAKHURNA_LIVE.serviceComeAr
          ...
          <div className="p-5 sm:p-6">
            <DialogHeader>
              <div className="flex items-center justify-between gap-3">
                <div>
                  <DialogTitle className="text-2xl font-black text-[#2a2016]">مراجعة السلة</DialogTitle>
                  <DialogDescription className="mt-2 text-sm leading-6 text-[#79674f]">
                    {STORE_BAKHURNA_LIVE.priceEstimateNoteAr}
                  </DialogDescription>
                </div>
                ...
                return (
                  <div key={line.catalogId} className="flex items-center gap-3 rounded-xl border border-[#e2d2b4] bg-white p-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-black text-[#2a2016]">{line.nameAr}</p>
            ...
            <div className="mt-6 rounded-xl bg-[#f3e6cf] p-4">
              <div className="flex items-center justify-between text-lg font-black text-[#2a2016]">
                <span>الإجمالي التقديري</span>
                <span>{total} ر.س</span>
              </div>
              <p className="mt-2 text-xs leading-5 text-[#79674f]">{STORE_BAKHURNA_LIVE.priceEstimateNoteAr}</p>
            </div>
            <Button
              type="button"
              onClick={() => {
              ...
              className="bakhurna-chatly-btn-primary mt-5 h-12 w-full rounded-xl text-sm font-black"
            >
              أكمل الطلب
              <ArrowLeft size={16} />
            </Button>
        ...
        <DialogContent className="max-w-xl border-[#dac8aa] bg-[#fdf9f0] text-right" dir="rtl">
            ...
            {services.map(({ id, label, icon: Icon, copy }) => (
              <Button
                ...
                className={cn(
                  ...
                  service === id
                    ? 'border-[#6e4a26] bg-[#f3e6cf]'
                    : 'border-[#e2d2b4] bg-white hover:bg-[#faf1de]',
                )}
              >
                <span className="flex size-9 items-center justify-center rounded-lg bg-[#fdf9f0] text-[#8a6239]">
                  <Icon size={18} />
                </span>
                <span className="text-sm font-black text-[#2a2016]">{label}</span>
          ...
          {service === 'pickup' && state.host.pickupPlaceVisible && state.host.pickupMapsUrl ? (
            <div className="mt-5 rounded-xl border border-[#dac8aa] bg-[#f3e6cf] p-4">
              ...
              >
                {STORE_BAKHURNA_LIVE.pickupPlaceOpenAr}
              </a>
            </div>
          ) : null}
          <Button
            ...
            className="bakhurna-chatly-btn-primary mt-6 h-11 w-full rounded-xl text-sm font-black"
          >
            تأكيد طريقة الخدمة
            <Check size={16} />
          </Button>
      ...
      <Dialog open={checkoutOpen} onOpenChange={setCheckoutOpen}>
        <DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto border-[#dac8aa] bg-[#fdf9f0] text-right" dir="rtl">
          <DialogHeader>
            ...
            <DialogDescription className="mt-2 text-sm leading-6 text-[#79674f]">
              الإجمالي الآن: {total} ر.س — {STORE_BAKHURNA_LIVE.priceEstimateNoteAr}
            </DialogDescription>
          </DialogHeader>
          <form
          ...
          >
            <label className="block text-xs font-bold text-[#6f6250]">
              {STORE_BAKHURNA_LIVE.buyerNameLabelAr}
              <Input
                required
                value={name}
            ...
            <label className="block text-xs font-bold text-[#6f6250]">
              {STORE_BAKHURNA_LIVE.buyerPhoneLabelAr}
              <Input
                required
                value={phone}
              ...
              {services.map(({ id, label }) => (
                <Button
                  ...
                  className={cn(
                    ...
                    service === id ? 'bakhurna-chatly-btn-primary' : 'bakhurna-chatly-btn-ghost',
                  )}
                >
                  {label}
                </Button>
            ...
            {service === 'delivery' || service === 'come' ? (
              <>
                <label className="block text-xs font-bold text-[#6f6250]">
                  {mobile ? STORE_MOBILE_VENDOR.placeHintAr : STORE_BAKHURNA_LIVE.buyerPlaceLabelAr}
                  <Input
                    value={place}
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                ...
                className={cn(
                  'h-9 rounded-full px-3 text-xs font-bold shadow-none',
                  pay === 'cash' ? 'bakhurna-chatly-btn-primary' : 'bakhurna-chatly-btn-ghost',
                )}
              >
                {STORE_BAKHURNA_LIVE.payCashAr}
              </Button>
              <Button
                type="button"
                onClick={() => setPay('card')}
                className={cn(
                  'h-9 rounded-full px-3 text-xs font-bold shadow-none',
                  pay === 'card' ? 'bakhurna-chatly-btn-primary' : 'bakhurna-chatly-btn-ghost',
                )}
              >
                {STORE_BAKHURNA_LIVE.payCardAr}
              </Button>
            </div>
            <div>
              <StoreDirectPayPublicMount product="store_bakhurna_live" token={token} accent={STORE_BAKHURNA_LIVE_ACCENT} />
            ...
            <label className="flex items-start gap-2 text-sm leading-7 text-[#79674f]">
              ...
              <span>{STORE_BAKHURNA_LIVE.saveBuyerAr}</span>
            </label>
            <Button type="submit" className="bakhurna-chatly-btn-primary h-12 w-full rounded-xl text-sm font-black">
              {service === 'come' ? STORE_BAKHURNA_LIVE.comeSubmitAr : STORE_BAKHURNA_LIVE.submitOrderAr}
            </Button>
      ...
      {notice ? (
        <div className="fixed bottom-24 right-4 z-50 flex items-center gap-2 rounded-full bg-[#2a2016] px-5 py-3 text-sm font-bold text-white shadow-xl sm:right-8">
...
function ProductCard({
  ...
  return (
    <article className="group overflow-hidden rounded-2xl border border-[#e2d2b4] bg-[#fdf9f0] shadow-[0_10px_25px_rgba(42,32,22,0.05)]">
      <ProductVisual item={item} imageIndex={imageIndex} />
      <div className="p-4">
        <div className="flex items-start justify-between gap-2">
          <div>
            <h3 className="text-base font-black text-[#2a2016]">{item.nameAr}</h3>
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          >
            {item.arrivedToday ? STORE_BAKHURNA_LIVE.arrivedOnAr : item.featured ? STORE_BAKHURNA_LIVE.featuredTitleAr : STORE_BAKHURNA_LIVE.stockOnAr}
          </span>
          {quantity ? (
            ...
            <Button
              type="button"
              onClick={() => onChange(1)}
              className="h-9 rounded-lg border-0 bg-[#f3e6cf] px-3 text-xs font-black text-[#6e4a26] shadow-none hover:bg-[#e9dcc0]"
            >
              إضافة سريعة
              <Plus size={14} />
            </Button>
  ...
  return (
    <div className={cn('relative overflow-hidden bg-[#e9dcc0]', small ? 'size-16 shrink-0 rounded-lg' : '')}>
      ...
      {!small && item.arrivedToday ? (
        ...
        <span className="absolute right-3 top-3 rounded-full bg-[#fdf9f0]/90 px-2 py-1 text-[10px] font-black text-[#8a6239]">
          {STORE_BAKHURNA_LIVE.featuredTitleAr}
        </span>
      ) : null}
    </div>
  );
...
function QtyControls({
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg bg-[#f3e6cf] px-2 py-1">
      <Button type="button" onClick={onMinus} className="size-7 rounded-md bg-transparent p-0 text-[#6e4a26] shadow-none hover:bg-white">
        <Minus size={14} />
      </Button>
      <span className="w-4 text-center text-sm font-black">{value}</span>
      <Button type="button" onClick={onPlus} className="size-7 rounded-md bg-transparent p-0 text-[#6e4a26] shadow-none hover:bg-white">
        <Plus size={14} />
      </Button>
    </div>
  );
...
function Feature({ icon: Icon, title, copy }: { icon: typeof Truck; title: string; copy: string }) {
  return (
    <div className="flex gap-3 rounded-2xl border border-[#e2d2b4] bg-[#fdf9f0] p-4">
      <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-[#f3e6cf] text-[#8a6239]">
        <Icon size={17} />
      </span>
      <div>
        <p className="text-sm font-black text-[#2a2016]">{title}</p>
