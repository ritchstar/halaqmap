...
export function VisitorServiceIntentRail({
  ...
  return (
    <div
      className={cn(
        isVertical
          ? 'w-[7.25rem] shrink-0 rounded-xl border border-[#dac8aa] bg-[#fbf6ec] px-2 py-3'
          : compact
            ? 'rounded-xl border border-[#dac8aa] bg-[#fbf6ec] p-2'
            : cn(
                'rounded-2xl border border-[#dac8aa] bg-gradient-to-b from-white/[0.06] to-transparent',
                'p-4 sm:p-5',
              ),
      )}
      dir="rtl"
    >
      ...
      >
        ...
        {compact ? (
          <span className="shrink-0 text-[0.58rem] font-semibold text-[#6f6250]">
            اسحب ←
          </span>
        ) : (
                ...
                isVertical
                  ? cn(
                      ...
                      active
                        ...
                        : 'border-[#dac8aa] bg-[#fbf6ec] hover:border-[#c9b48c] hover:bg-[#f0e6d0]',
                    )
                  : cn(
      ...
      {!isVertical && !compact ? (
        <p className="mt-3 text-[0.68rem] leading-relaxed text-[#6f6250]">
          {hasLocation ? VISITOR_INTENT_RAIL_HINT_WITH_LOCATION_AR : VISITOR_INTENT_RAIL_HINT_NO_LOCATION_AR}
        </p>
      ) : isVertical ? (
        <p className="mt-2 text-center text-[0.52rem] leading-snug text-[#6f6250]">
          {hasLocation ? 'نشطة' : 'ثم الموقع'}
        </p>
      ) : compact ? (
        <p className="mt-2 text-[0.62rem] leading-relaxed text-[#6f6250]">
          {hasLocation ? VISITOR_INTENT_RAIL_HINT_WITH_LOCATION_AR : VISITOR_INTENT_RAIL_HINT_NO_LOCATION_AR}
        </p>
      ) : null}
    </div>
