/**
 * Pulse Map — southern pilot slot registry (client copy).
...
export function outlineToSvgPath(vertices: ReadonlyArray<[number, number]>): string {
  const parts: string[] = [];
...
export const PULSE_MAP_SLOT_BY_CITY_ID = new Map(
  SOUTHERN_SLOT_SEEDS.map((s) => [s.id, s.id]),
);
