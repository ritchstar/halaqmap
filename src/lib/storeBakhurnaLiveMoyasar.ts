 ...
 */
import { STORE_BAKHURNA_LIVE_PRODUCT } from '@/config/storeBakhurnaLive';

export function bakhurnaLiveLivePaymentsEnabled(): boolean {
  const liveEnv = String(import.meta.env.VITE_PAYMENT_ENV || 'test').trim().toLowerCase() === 'live';
  const raw = String(
    import.meta.env.VITE_STORE_BAKHURNA_LIVE_LIVE_PAYMENTS ?? import.meta.env.VITE_STORE_PAID_INVITE_LIVE_PAYMENTS ?? '',
  )
    .trim()
    .toLowerCase();
...
export function resolveBakhurnaLivePublishableKey(): string {
  ...
  if (bakhurnaLiveLivePaymentsEnabled()) {
...
export function buildBakhurnaLiveCallbackUrl(token: string): string {
  const origin =
...
export function readBakhurnaLiveReturnPaymentId(): string {
  return returnParams().get('id')?.trim() || '';
}

export function isBakhurnaLivePaymentReturn(): boolean {
  const params = returnParams();
  if ((params.get('purpose') || '').trim() === STORE_BAKHURNA_LIVE_PRODUCT) return true;
