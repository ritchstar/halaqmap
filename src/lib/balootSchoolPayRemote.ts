 ...
 */
/**
 * عميل دفع مدرسة البلوت — يتحدث مع /api/baloot-school-pay فقط.
 */
const PAY_API_PATH = '/api/baloot-school-pay';
...
function balootSchoolPayEndpoint(): string {
...
async function postAction(body: Record<string, unknown>): Promise<{ ok: boolean; error?: string; [k: string]: unknown }> {
  try {
    const res = await fetch(balootSchoolPayEndpoint(), {
      method: 'POST',
...
export async function fetchBalootSchoolPay(registrationId: string) {
  return postAction({ action: 'get_public', registrationId });
}

export async function activateBalootSchoolPay(registrationId: string, paymentId: string) {
