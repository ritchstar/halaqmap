 ...
 */
const PATH = '/api/public-store-bakhurna-gift';

export type StoreBakhurnaGiftPublicState = {
  slotNo: number;
  slotCount: number;
...
export async function fetchStoreBakhurnaGiftState(): Promise<StoreBakhurnaGiftPublicState | null> {
  try {
    ...
    return {
      ...
      nominees: Array.isArray(data.nominees)
        ? (data.nominees as StoreBakhurnaGiftPublicState['nominees'])
        : [],
    };
  } catch {
    return null;
...
export async function enterStoreBakhurnaGift(body: Record<string, unknown>): Promise<{ ok: boolean; error?: string }> {
  try {
...
export async function confirmStoreBakhurnaGift(token: string): Promise<{ ok: boolean; error?: string }> {
  try {
    const res = await fetch(PATH, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
