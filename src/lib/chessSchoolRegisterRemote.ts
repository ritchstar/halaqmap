 ...
 */
/**
 * تسجيل مدرسة الشطرنج الاحترافية — عميل واجهة يتحدث مع /api/chess-school-register
 * و/api/chess-school-confirm-email فقط (بلا Supabase مباشرة من المتصفح).
 */
...
const REGISTER_URL = String(
  import.meta.env.VITE_CHESS_SCHOOL_REGISTER_URL || '/api/chess-school-register',
).trim();
const CONFIRM_URL = String(
  import.meta.env.VITE_CHESS_SCHOOL_CONFIRM_URL || '/api/chess-school-confirm-email',
).trim();

export async function submitChessSchoolRegistrationRemote(input: {
  fullName: string;
  email: string;
...
export function chessSchoolRegisterErrorAr(code: string): string {
  switch (code) {
    case 'invalid_email':
...
export function chessSchoolConfirmErrorAr(code: string): string {
  switch (code) {
    case 'token_expired':
