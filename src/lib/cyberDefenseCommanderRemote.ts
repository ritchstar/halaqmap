...
const LAB_CHAT_API = '/api/admin-cyber-defense-commander-lab-chat';
export const CYBER_DEFENSE_LAB_TIMEOUT_MS = 60_000;

export type CyberDefensePostureLevel = 'green' | 'amber' | 'red';

export type CyberDefenseSnapshot = {
...
export async function fetchCyberDefenseDiagnostics(): Promise<
...
export async function chatWithCyberDefenseCommander(
  input: {
    userMessage?: string;
    imageBase64?: string;
    imageMime?: string;
  ...
  | { ok: true; reply: string; defenseContext?: CyberDefenseSnapshot }
...
> {
  ...
  const timeoutId = setTimeout(() => controller.abort(), CYBER_DEFENSE_LAB_TIMEOUT_MS);
  ...
  try {
    ...
    if (!reply) return { ok: false, error: 'رد فارغ من القائد الأعلى للدفاع' };
    const defenseContext = (json.defenseContext as CyberDefenseSnapshot | undefined) ?? undefined;
    return { ok: true, reply, defenseContext };
  } catch (e) {
