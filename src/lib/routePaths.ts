...
export const ROUTE_PATHS = {
  ...
  STORE_GENERAL_TRIAL_NOT_ME: '/store/try/not-me',
   ...
   * المرحلة ١ (هذه): تسجيل بالاسم والبريد + تأكيد بريد حقيقي. مرحلتا الدفع
   * (Moyasar الفعلي) والصفحة الخاصة الدائمة تُبنَيان لاحقاً على هذا الأساس.
   */
  ...
  /**
   * ساحة بلوت — المرحلة الأولى التأسيسية: لعب حكم (بلا صن أو مضاعفة بعد)
   * فريق حقيقي ٢ ضد ٢ (اللاعب + شريك آلي مقابل خصمين آليين)، مجاني بالكامل
   * كساحة تجربة (بلا منتج مدفوع بعد)، بجلسة محفوظة محلياً. صن، المضاعفة،
   * والتعدد الحقيقي بين لاعبين بشريين مراحل لاحقة.
   */
  BALOOT_ARENA: '/baloot',
  /**
   * مدرسة البلوت — منتج تعليمي رقمي مستقل (199 ر.س، وصول دائم)، على نفس
   * مسار مدرسة الشطرنج تماماً: تسجيل بالاسم والبريد → تأكيد بريد حقيقي →
   * دفع Moyasar فعلي → صفحة طالب خاصة غير مفهرسة تضم المنهج الأساسي
   * (القواعد، ترتيب الورق، الحسبة) مبنياً على محرك قواعد بلوت الثابت
   * (`balootEngine.ts`) — لا قواعد مُخترعة، ولا وعود احتراف أو فوز، ولا
   * مباريات عامة أو جوائز أو رهانات في هذه المرحلة.
   */
  BALOOT_SCHOOL_LANDING: '/baloot/school',
  /** تأكيد بريد تسجيل مدرسة البلوت (?c=token) */
  BALOOT_SCHOOL_CONFIRM: '/baloot/school/confirm',
  /**
   * دفع مدرسة البلوت الحقيقي عبر Moyasar (199 ر.س، دفعة واحدة). لا تُفتح
   * إلا بعد تأكيد البريد؛ :rid هو معرّف التسجيل نفسه.
   */
  BALOOT_SCHOOL_PAY: '/baloot/school/pay/:rid',
  /**
   * الصفحة الخاصة الدائمة لطالب مدرسة البلوت بعد الدفع — غير عامة وغير
   * مفهرسة (noindex)؛ :rid هو معرّف التسجيل نفسه (UUID غير قابل للتخمين).
   */
  BALOOT_SCHOOL_STUDENT: '/baloot/school/student/:rid',
