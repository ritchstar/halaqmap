 ...
 */
import { STORE_RESTAURANT_LIVE_PRODUCT } from '@/config/storeRestaurantLive';

export function restaurantLiveLivePaymentsEnabled(): boolean {
  const liveEnv = String(import.meta.env.VITE_PAYMENT_ENV || 'test').trim().toLowerCase() === 'live';
  const raw = String(
    import.meta.env.VITE_STORE_RESTAURANT_LIVE_LIVE_PAYMENTS ?? import.meta.env.VITE_STORE_PAID_INVITE_LIVE_PAYMENTS ?? '',
  )
    .trim()
    .toLowerCase();
...
export function resolveRestaurantLivePublishableKey(): string {
  ...
  if (restaurantLiveLivePaymentsEnabled()) {
...
export function buildRestaurantLiveCallbackUrl(token: string): string {
  const origin =
...
export function readRestaurantLiveReturnPaymentId(): string {
  return returnParams().get('id')?.trim() || '';
}

export function isRestaurantLivePaymentReturn(): boolean {
  const params = returnParams();
  if ((params.get('purpose') || '').trim() === STORE_RESTAURANT_LIVE_PRODUCT) return true;
