...
const BAKHURNA_LIVE_API_PATH = '/api/public-store-bakhurna-live';
...
export function storeBakhurnaLiveEndpoint(): string {
  if (typeof window !== 'undefined') {
    const host = window.location.hostname.toLowerCase();
    if (LIVE_API_HOSTS.has(host)) return BAKHURNA_LIVE_API_PATH;
  }
  const origin = configuredApiOrigin();
  if (origin && !/\.vercel\.app$/i.test(origin)) return `${origin}${BAKHURNA_LIVE_API_PATH}`;
  return BAKHURNA_LIVE_API_PATH;
}
...
async function postAction(body: Record<string, unknown>): Promise<{ ok: boolean; error?: string; [k: string]: unknown }> {
  try {
    const res = await fetch(storeBakhurnaLiveEndpoint(), {
      method: 'POST',
...
export async function createBakhurnaLivePending(input: Record<string, unknown>) {
  return postAction({ action: 'create_pending', ...input });
}

export async function activateBakhurnaLive(token: string, paymentId: string) {
  return postAction({ action: 'activate_paid', token, paymentId });
}

export async function syncBakhurnaLive(token: string) {
  return postAction({ action: 'sync_paid', token });
}

export async function fetchBakhurnaLivePay(token: string) {
  return postAction({ action: 'get_public', token, role: 'pay' });
}

export async function fetchBakhurnaLivePublic(token: string, role: 'shop' | 'desk') {
  return fetchStoreLivePublicGet(storeBakhurnaLiveEndpoint(), token, role);
}

export async function addBakhurnaLiveOrder(token: string, order: Record<string, unknown>) {
  return postAction({ action: 'add_order', token, order });
}

export async function saveBakhurnaLiveHost(input: Record<string, unknown>) {
  return postAction({ action: 'save_host', ...input });
}

export async function addBakhurnaLiveChat(token: string, chat: Record<string, unknown>) {
