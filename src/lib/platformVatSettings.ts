 ...
 * الافتراضي: معطّلة (عمل حر / غير خاضع) — المبالغ المعروضة = رسوم الاشتراك فقط.
...
export function getSubscriptionPricingVatClauseAr(settings: PlatformVatSettings): string {
  if (!settings.enabled) {
    return 'الأسعار المعروضة بالريال السعودي تمثل رسوم الاشتراك في المنصة فقط، دون ضريبة قيمة مضافة في الوقت الحالي. عند التوسع بسجل تجاري واستحقاق الضريبة يمكن تفعيل احتسابها تلقائياً من إعدادات الإدارة بالنسبة المعتمدة وقتها.';
  }
  return `الأسعار المعروضة بالريال السعودي تمثل رسوم الاشتراك الأساسية؛ تُضاف ضريبة القيمة المضافة بنسبة ${settings.ratePercent}% على مبلغ الدفع عند إتمامه، وفق تفعيل الإدارة ويمكن تعديل النسبة لاحقاً بما يتوافق مع الأنظمة.`;
}
