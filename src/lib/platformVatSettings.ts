 ...
 * الافتراضي: معطّلة (عمل حر / غير خاضع) — المبالغ المعروضة = قيمة الترخيص الرقمي فقط.
...
export function getSubscriptionPricingVatClauseAr(settings: PlatformVatSettings): string {
  if (!settings.enabled) {
    return 'الأسعار المعروضة بالريال السعودي تمثل قيمة الترخيص الرقمي لخدمات الإدراج البرمجية فقط، دون ضريبة قيمة مضافة في الوقت الحالي. عند التوسع بسجل تجاري واستحقاق الضريبة يمكن تفعيل احتسابها تلقائياً من إعدادات الإدارة بالنسبة المعتمدة وقتها.';
  }
  return `الأسعار المعروضة بالريال السعودي تمثل قيمة الترخيص الرقمي الأساسية؛ تُضاف ضريبة القيمة المضافة بنسبة ${settings.ratePercent}% على مبلغ الدفع عند إتمامه، وفق تفعيل الإدارة ويمكن تعديل النسبة لاحقاً بما يتوافق مع الأنظمة.`;
}
