...
export function getSubscriptionPricingVatClauseAr(settings: PlatformVatSettings): string {
  if (!settings.enabled) {
    return 'الأسعار المعروضة بالريال السعودي تمثل قيمة حزمة رخصة النفاذ الرقمية (نظام الاستجابة الذكية) فقط، دون ضريبة قيمة مضافة في الوقت الحالي. عند التوسع بسجل تجاري واستحقاق الضريبة يمكن تفعيل احتسابها تلقائياً من إعدادات الإدارة بالنسبة المعتمدة وقتها.';
  }
