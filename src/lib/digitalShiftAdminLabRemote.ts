...
const LAB_CHAT_API = '/api/admin-digital-shift-lab-chat';
export const DIGITAL_SHIFT_LAB_TIMEOUT_MS = 55_000;
...
export async function fetchDigitalShiftLabDiagnostics(): Promise<
...
export async function chatWithDigitalShiftLab(
  input: {
    userMessage?: string;
    imageBase64?: string;
    imageMime?: string;
  ...
  const timeoutId = setTimeout(() => controller.abort(), DIGITAL_SHIFT_LAB_TIMEOUT_MS);
