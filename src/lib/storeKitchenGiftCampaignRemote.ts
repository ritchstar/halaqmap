 ...
 */
const PATH = '/api/public-store-kitchen-gift';

export type StoreKitchenGiftPublicState = {
  slotNo: number;
  slotCount: number;
...
export async function fetchStoreKitchenGiftState(): Promise<StoreKitchenGiftPublicState | null> {
  try {
    ...
    return {
      ...
      nominees: Array.isArray(data.nominees)
        ? (data.nominees as StoreKitchenGiftPublicState['nominees'])
        : [],
    };
  } catch {
    return null;
...
export async function enterStoreKitchenGift(body: Record<string, unknown>): Promise<{ ok: boolean; error?: string }> {
  try {
...
export async function confirmStoreKitchenGift(token: string): Promise<{ ok: boolean; error?: string }> {
  try {
    const res = await fetch(PATH, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
