 ...
 */
import { STORE_PRODUCE_LIVE_PRODUCT } from '@/config/storeProduceLive';

export function produceLiveLivePaymentsEnabled(): boolean {
  const liveEnv = String(import.meta.env.VITE_PAYMENT_ENV || 'test').trim().toLowerCase() === 'live';
  const raw = String(
    import.meta.env.VITE_STORE_PRODUCE_LIVE_LIVE_PAYMENTS ?? import.meta.env.VITE_STORE_PAID_INVITE_LIVE_PAYMENTS ?? '',
  )
    .trim()
    .toLowerCase();
...
export function resolveProduceLivePublishableKey(): string {
  ...
  if (produceLiveLivePaymentsEnabled()) {
...
export function buildProduceLiveCallbackUrl(token: string): string {
  const origin =
...
export function readProduceLiveReturnPaymentId(): string {
  return returnParams().get('id')?.trim() || '';
}

export function isProduceLivePaymentReturn(): boolean {
  const params = returnParams();
  if ((params.get('purpose') || '').trim() === STORE_PRODUCE_LIVE_PRODUCT) return true;
