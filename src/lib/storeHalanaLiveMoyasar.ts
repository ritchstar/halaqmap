 ...
 */
import { STORE_HALANA_LIVE_PRODUCT } from '@/config/storeHalanaLive';

export function halanaLiveLivePaymentsEnabled(): boolean {
  const liveEnv = String(import.meta.env.VITE_PAYMENT_ENV || 'test').trim().toLowerCase() === 'live';
  const raw = String(
    import.meta.env.VITE_STORE_HALANA_LIVE_LIVE_PAYMENTS ?? import.meta.env.VITE_STORE_PAID_INVITE_LIVE_PAYMENTS ?? '',
  )
    .trim()
    .toLowerCase();
...
export function resolveHalanaLivePublishableKey(): string {
  ...
  if (halanaLiveLivePaymentsEnabled()) {
...
export function buildHalanaLiveCallbackUrl(token: string): string {
  const origin =
    typeof window !== 'undefined' ? window.location.origin.replace(/\/+$/, '') : 'https://www.halaqmap.com';
  const q = new URLSearchParams();
  q.set('purpose', STORE_HALANA_LIVE_PRODUCT);
  q.set('store_halana_token', token);
...
export function readHalanaLiveReturnPaymentId(): string {
  return returnParams().get('id')?.trim() || '';
}

export function isHalanaLivePaymentReturn(): boolean {
  const params = returnParams();
  if ((params.get('purpose') || '').trim() === STORE_HALANA_LIVE_PRODUCT) return true;
