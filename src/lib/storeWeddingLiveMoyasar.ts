 ...
 *
 * مفاتيح ميسر لدعوة الزواج — نفس بوابة المتجر، وسم مستقل.
 */
import { STORE_WEDDING_LIVE_PRODUCT } from '@/config/storeWeddingLive';

export function weddingLiveLivePaymentsEnabled(): boolean {
  const liveEnv = String(import.meta.env.VITE_PAYMENT_ENV || 'test').trim().toLowerCase() === 'live';
  const raw = String(import.meta.env.VITE_STORE_WEDDING_LIVE_LIVE_PAYMENTS ?? import.meta.env.VITE_STORE_PAID_INVITE_LIVE_PAYMENTS ?? '')
    .trim()
    .toLowerCase();
...
export function resolveWeddingLivePublishableKey(): string {
  ...
  if (weddingLiveLivePaymentsEnabled()) {
...
export function buildWeddingLiveCallbackUrl(token: string): string {
  const origin =
...
export function readWeddingLiveReturnPaymentId(): string {
  return returnParams().get('id')?.trim() || '';
}

export function isWeddingLivePaymentReturn(): boolean {
  const params = returnParams();
  if ((params.get('purpose') || '').trim() === STORE_WEDDING_LIVE_PRODUCT) return true;
