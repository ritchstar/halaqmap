 ...
 *
 * مفاتيح ميسر للدعوة الحرة — نفس بوابة المتجر، وسم مستقل.
 */
import { STORE_EVENT_LIVE_PRODUCT } from '@/config/storeEventLive';

export function eventLiveLivePaymentsEnabled(): boolean {
  const liveEnv = String(import.meta.env.VITE_PAYMENT_ENV || 'test').trim().toLowerCase() === 'live';
  const raw = String(import.meta.env.VITE_STORE_EVENT_LIVE_LIVE_PAYMENTS ?? import.meta.env.VITE_STORE_PAID_INVITE_LIVE_PAYMENTS ?? '')
    .trim()
    .toLowerCase();
...
export function resolveEventLivePublishableKey(): string {
  ...
  if (eventLiveLivePaymentsEnabled()) {
...
export function buildEventLiveCallbackUrl(token: string): string {
  const origin =
...
export function readEventLiveReturnPaymentId(): string {
  return returnParams().get('id')?.trim() || '';
}

export function isEventLivePaymentReturn(): boolean {
  const params = returnParams();
  if ((params.get('purpose') || '').trim() === STORE_EVENT_LIVE_PRODUCT) return true;
