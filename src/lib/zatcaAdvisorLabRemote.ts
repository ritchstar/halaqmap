...
const LAB_CHAT_API = '/api/admin-zatca-advisor-lab-chat';
export const ZATCA_ADVISOR_LAB_TIMEOUT_MS = 55_000;
...
export async function fetchZatcaAdvisorLabDiagnostics(): Promise<
...
export async function chatWithZatcaAdvisorLab(
  input: {
    userMessage?: string;
    imageBase64?: string;
    imageMime?: string;
  ...
  const timeoutId = setTimeout(() => controller.abort(), ZATCA_ADVISOR_LAB_TIMEOUT_MS);
