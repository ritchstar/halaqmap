...
export function storeRestaurantLiveEndpoint(): string {
  if (typeof window !== 'undefined') {
    const host = window.location.hostname.toLowerCase();
    if (LIVE_API_HOSTS.has(host)) return RESTAURANT_LIVE_API_PATH;
  }
  const origin = configuredApiOrigin();
  if (origin && !/\.vercel\.app$/i.test(origin)) return `${origin}${RESTAURANT_LIVE_API_PATH}`;
  return RESTAURANT_LIVE_API_PATH;
}
...
async function postAction(body: Record<string, unknown>): Promise<{ ok: boolean; error?: string; [k: string]: unknown }> {
  try {
    const res = await fetch(storeRestaurantLiveEndpoint(), {
      method: 'POST',
...
export async function createRestaurantLivePending(input: Record<string, unknown>) {
  return postAction({ action: 'create_pending', ...input });
}

export async function activateRestaurantLive(token: string, paymentId: string) {
  return postAction({ action: 'activate_paid', token, paymentId });
}

export async function syncRestaurantLive(token: string) {
  return postAction({ action: 'sync_paid', token });
}

export async function fetchRestaurantLivePay(token: string) {
...
export async function saveRestaurantLiveHost(input: Record<string, unknown>) {
  return postAction({ action: 'save_host', ...input });
}

export async function addRestaurantLiveChat(token: string, chat: Record<string, unknown>) {
