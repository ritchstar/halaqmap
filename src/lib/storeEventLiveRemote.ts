...
export async function claimEventGuestSeat(input: { token: string; seatId: string; inviteId?: string; deviceHash: string }) {
  return postAction({ action: 'claim_guest_seat', ...input });
}

export async function mintEventGuestInvite(token: string, count = 200) {
  return postAction({ action: 'mint_guest_invite', token, count });
}

export async function listEventGuestInvites(token: string) {
