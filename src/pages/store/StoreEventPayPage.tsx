 ...
 *
 * دفع الدعوة الحرة التفاعلية — 899 ر.س، وسم store_event_live.
 */
...
import {
  buildEventLiveCallbackUrl,
  isAllowedMoyasarInvoiceUrl,
  isEventLivePaymentReturn,
  readEventLiveReturnPaymentId,
  resolveEventLivePublishableKey,
  eventLiveLivePaymentsEnabled,
} from '@/lib/storeEventLiveMoyasar';
import { activateEventLive, fetchEventLivePay, syncEventLive } from '@/lib/storeEventLiveRemote';
import { eventLiveViewHref } from '@/lib/storeHostRedirect';
...
export default function StoreEventPayPage() {
  ...
  const publishableKey = useMemo(() => resolveEventLivePublishableKey(), []);
  const liveMoney = eventLiveLivePaymentsEnabled();
  const amountOk = priceHalalas === STORE_EVENT_LIVE_PRICE_HALALAS;

  useEffect(() => {
    let cancelled = false;
    void fetchEventLivePay(token).then((result) => {
      if (cancelled) return;
      if (!result.ok) {
        setStatus('missing');
  ...
  useEffect(() => {
    ...
    if (!isEventLivePaymentReturn()) return;
    const paymentId = readEventLiveReturnPaymentId();
    const hasInvoice = isAllowedMoyasarInvoiceUrl(invoiceUrl);
    if (!paymentId && !hasInvoice) return;
    let cancelled = false;
    setActivating(true);
    const run = paymentId ? activateEventLive(token, paymentId) : syncEventLive(token);
    void run.then((result) => {
      if (cancelled) return;
      const finishOk = () => {
        activateOnceRef.current = true;
        window.location.replace(eventLiveViewHref(token));
      };
      if (result.ok) {
        finishOk();
        return;
      ...
      if (paymentId && hasInvoice) {
        void syncEventLive(token).then((synced) => {
          if (cancelled) return;
          if (synced.ok) {
            finishOk();
            return;
      ...
      .then(() => {
        ...
        Moyasar.init({
          ...
          on_completed: (payment: unknown) => {
            ...
            activateOnceRef.current = true;
            setActivating(true);
            void activateEventLive(token, id).then((result) => {
              if (result.ok) {
                window.location.replace(eventLiveViewHref(token));
                return;
              }
      ...
      <main className="mx-auto max-w-lg px-4 py-12">
        ...
        {status === 'live' ? (
          <p className="mt-6 text-sm text-white/70">
            الدعوة حيّة.{' '}
            <a className="underline" href={eventLiveViewHref(token)}>
              فتح شاشة القاعة
            </a>
          </p>
        ) : null}
        {status === 'pending_payment' && STORE_EVENT_LIVE_CHECKOUT_ENABLED ? (
          <>
            <p className="mt-3 text-sm leading-7 text-white/70">
              {hostName || STORE_EVENT_LIVE.titleAr} — {STORE_EVENT_LIVE_PRICE_SAR} ر.س
            </p>
            {!liveMoney ? (
        ...
        <Link to={ROUTE_PATHS.STORE_EVENT} className="mt-8 inline-block text-sm text-white/50 underline">
          {STORE_EVENT_LIVE.titleAr}
        </Link>
      </main>
    </div>
  );
