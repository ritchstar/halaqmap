 ...
 *
 * دفع مطعمنا1 — 699 أو 999 ر.س، وسم store_restaurant_live.
 */
...
import {
  buildRestaurantLiveCallbackUrl,
  isAllowedMoyasarInvoiceUrl,
  isRestaurantLivePaymentReturn,
  readRestaurantLiveReturnPaymentId,
  resolveRestaurantLivePublishableKey,
  restaurantLiveLivePaymentsEnabled,
} from '@/lib/storeRestaurantLiveMoyasar';
import { activateRestaurantLive, fetchRestaurantLivePay, syncRestaurantLive } from '@/lib/storeRestaurantLiveRemote';
import { restaurantLiveViewHref } from '@/lib/storeHostRedirect';
...
function priceLabel(halalas: number): string {
  if (halalas === STORE_RESTAURANT_LIVE_PRICE_12_HALALAS) return '999 ر.س لاثني عشر شهراً';
  if (halalas === STORE_RESTAURANT_LIVE_PRICE_6_HALALAS) return '699 ر.س لستة أشهر';
  return '';
}

function invoiceDescription(halalas: number): string {
  return halalas === STORE_RESTAURANT_LIVE_PRICE_12_HALALAS
    ? 'halaqmap — مطعمنا1 12 شهراً'
    : 'halaqmap — مطعمنا1 6 أشهر';
}

export default function StoreRestaurantPayPage() {
  useDocumentTitle('دفع مطعمنا1 — halaqmap');
  ...
  const publishableKey = useMemo(() => resolveRestaurantLivePublishableKey(), []);
  const liveMoney = restaurantLiveLivePaymentsEnabled();
  const amountOk =
  ...
  useEffect(() => {
    let cancelled = false;
    void fetchRestaurantLivePay(token).then((result) => {
      if (cancelled) return;
      if (!result.ok) {
        setStatus('missing');
  ...
  useEffect(() => {
    ...
    if (!isRestaurantLivePaymentReturn()) return;
    const paymentId = readRestaurantLiveReturnPaymentId();
    const hasInvoice = isAllowedMoyasarInvoiceUrl(invoiceUrl);
    if (!paymentId && !hasInvoice) return;
    let cancelled = false;
    setActivating(true);
    const run = paymentId ? activateRestaurantLive(token, paymentId) : syncRestaurantLive(token);
    void run.then((result) => {
      if (cancelled) return;
      const finishOk = () => {
        activateOnceRef.current = true;
        window.location.replace(restaurantLiveViewHref(token));
      };
      if (result.ok) {
        finishOk();
        return;
      ...
      if (paymentId && hasInvoice) {
        void syncRestaurantLive(token).then((synced) => {
          if (cancelled) return;
          if (synced.ok) {
            finishOk();
            return;
    ...
    void loadMoyasarFormScript()
      .then(() => {
        ...
        Moyasar.init({
          ...
          callback_url: buildRestaurantLiveCallbackUrl(token),
          ...
          on_completed: (payment: unknown) => {
            ...
            activateOnceRef.current = true;
            setActivating(true);
            void activateRestaurantLive(token, id).then((result) => {
              if (result.ok) {
                window.location.replace(restaurantLiveViewHref(token));
                return;
              }
  ...
  return (
    <div dir="rtl" className="min-h-[100svh] bg-[#061018] text-[#f4efe4]">
      <main className="mx-auto max-w-lg px-4 py-12">
        <p className="text-sm font-bold text-[#e08a3c]">halaqmap</p>
        <h1 className="mt-2 text-2xl font-extrabold">دفع مطعمنا1</h1>
        {!STORE_RESTAURANT_LIVE_CHECKOUT_ENABLED ? (
        ...
        {status === 'live' ? (
          <p className="mt-6 text-sm text-white/70">
            التشغيل حيّ.{' '}
            <a className="underline" href={restaurantLiveViewHref(token)}>
              فتح صفحة المطعم
            </a>
          </p>
        ) : null}
        {payable && STORE_RESTAURANT_LIVE_CHECKOUT_ENABLED ? (
          <>
            <p className="mt-3 text-sm leading-7 text-white/70">
              {shopName || STORE_RESTAURANT_LIVE.titleAr} — {priceLabel(priceHalalas)}
            </p>
            {!liveMoney ? (
        ...
        <Link to={ROUTE_PATHS.STORE_RESTAURANT} className="mt-8 inline-block text-sm text-white/50 underline">
          {STORE_RESTAURANT_LIVE.titleAr}
        </Link>
      </main>
    </div>
  );
