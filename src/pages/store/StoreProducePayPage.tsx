 ...
 *
 * دفع خضارنا1 — 1350 أو 2500 ر.س، وسم store_produce_live.
 */
...
import {
  ...
  isProduceLivePaymentReturn,
  produceLiveLivePaymentsEnabled,
  readProduceLiveReturnPaymentId,
  resolveProduceLivePublishableKey,
} from '@/lib/storeProduceLiveMoyasar';
import { activateProduceLive, fetchProduceLivePay, syncProduceLive } from '@/lib/storeProduceLiveRemote';
import { produceLiveViewHref } from '@/lib/storeHostRedirect';
...
function priceLabel(halalas: number): string {
  if (halalas === STORE_PRODUCE_LIVE_PRICE_12_HALALAS) return '2500 ر.س لثلاثمئة وستين يوماً';
  if (halalas === STORE_PRODUCE_LIVE_PRICE_6_HALALAS) return '1350 ر.س لمئة وثمانين يوماً';
  return '';
}

function invoiceDescription(halalas: number): string {
  return halalas === STORE_PRODUCE_LIVE_PRICE_12_HALALAS
    ? 'halaqmap — خضارنا1 360 يوماً'
    : 'halaqmap — خضارنا1 180 يوماً';
}

export default function StoreProducePayPage() {
  useDocumentTitle('دفع خضارنا1 — halaqmap');
  ...
  const publishableKey = useMemo(() => resolveProduceLivePublishableKey(), []);
  const liveMoney = produceLiveLivePaymentsEnabled();
  const amountOk =
  ...
  useEffect(() => {
    let cancelled = false;
    void fetchProduceLivePay(token).then((result) => {
      if (cancelled) return;
      if (!result.ok) {
        setStatus('missing');
  ...
  useEffect(() => {
    ...
    if (!isProduceLivePaymentReturn()) return;
    const paymentId = readProduceLiveReturnPaymentId();
    ...
    void run.then((result) => {
      ...
      if (paymentId && hasInvoice) {
        void syncProduceLive(token).then((synced) => {
          if (cancelled) return;
          if (synced.ok) {
            finishOk();
            return;
    ...
    void loadMoyasarFormScript()
      .then(() => {
        ...
        Moyasar.init({
          ...
          callback_url: buildProduceLiveCallbackUrl(token),
  ...
  return (
    <div dir="rtl" className="min-h-[100svh] bg-[#061018] text-[#f4efe4]">
      <main className="mx-auto max-w-lg px-4 py-12">
        <p className="text-sm font-bold text-[#3d8b4a]">halaqmap</p>
        <h1 className="mt-2 text-2xl font-extrabold">دفع خضارنا1</h1>
        {!STORE_PRODUCE_LIVE_CHECKOUT_ENABLED ? (
          <p className="mt-3 text-sm leading-7 text-white/70">{STORE_PRODUCE_LIVE.checkoutClosedAr}</p>
        ) : null}
        ...
        {status === 'live' ? (
          <p className="mt-6 text-sm leading-7 text-white/70">
            التشغيل حيّ.{' '}
            <a className="underline" href={produceLiveViewHref(token)}>
              فتح صفحة الصندوق
            </a>
          </p>
        ) : null}
        {payable && STORE_PRODUCE_LIVE_CHECKOUT_ENABLED ? (
          <>
            <p className="mt-3 text-sm leading-7 text-white/70">
              {shopName || STORE_PRODUCE_LIVE.titleAr} — {priceLabel(priceHalalas)}
            </p>
            {!liveMoney ? (
              <p className="mt-3 rounded-xl border border-[#3d8b4a]/30 bg-[#3d8b4a]/10 px-3 py-2 text-xs leading-6">
            ...
            {hostedInvoice && !activating ? (
              <a
                href={invoiceUrl}
                className="mt-6 inline-flex w-full items-center justify-center rounded-xl bg-[#3d8b4a] px-4 py-3 text-sm font-bold text-[#061018]"
              >
        ...
        <Link to={ROUTE_PATHS.STORE_PRODUCE} className="mt-8 inline-block text-sm text-white/50 underline">
