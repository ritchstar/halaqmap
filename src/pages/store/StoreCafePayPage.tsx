 ...
 *
 * دفع كافينا1 — 1199 أو 2099 ر.س، وسم store_cafe_live.
 */
...
import {
  buildCafeLiveCallbackUrl,
  isAllowedMoyasarInvoiceUrl,
  isCafeLivePaymentReturn,
  readCafeLiveReturnPaymentId,
  resolveCafeLivePublishableKey,
  cafeLiveLivePaymentsEnabled,
} from '@/lib/storeCafeLiveMoyasar';
import { activateCafeLive, fetchCafeLivePay, syncCafeLive } from '@/lib/storeCafeLiveRemote';
import { cafeLiveViewHref } from '@/lib/storeHostRedirect';
...
function priceLabel(halalas: number): string {
  if (halalas === STORE_CAFE_LIVE_PRICE_12_HALALAS) return '2099 ر.س لاثني عشر شهراً';
  if (halalas === STORE_CAFE_LIVE_PRICE_6_HALALAS) return '1199 ر.س لستة أشهر';
  return '';
}

function invoiceDescription(halalas: number): string {
  return halalas === STORE_CAFE_LIVE_PRICE_12_HALALAS
    ? 'halaqmap — كافينا1 12 شهراً'
    : 'halaqmap — كافينا1 6 أشهر';
}

export default function StoreCafePayPage() {
  useDocumentTitle('دفع كافينا1 — halaqmap');
  ...
  const publishableKey = useMemo(() => resolveCafeLivePublishableKey(), []);
  const liveMoney = cafeLiveLivePaymentsEnabled();
  const amountOk =
  ...
  useEffect(() => {
    let cancelled = false;
    void fetchCafeLivePay(token).then((result) => {
      if (cancelled) return;
      if (!result.ok) {
        setStatus('missing');
  ...
  useEffect(() => {
    ...
    if (!isCafeLivePaymentReturn()) return;
    const paymentId = readCafeLiveReturnPaymentId();
    const hasInvoice = isAllowedMoyasarInvoiceUrl(invoiceUrl);
    if (!paymentId && !hasInvoice) return;
    let cancelled = false;
    setActivating(true);
    const run = paymentId ? activateCafeLive(token, paymentId) : syncCafeLive(token);
    void run.then((result) => {
      if (cancelled) return;
      const finishOk = () => {
        activateOnceRef.current = true;
        window.location.replace(cafeLiveViewHref(token));
      };
      if (result.ok) {
        finishOk();
        return;
      ...
      if (paymentId && hasInvoice) {
        void syncCafeLive(token).then((synced) => {
          if (cancelled) return;
          if (synced.ok) {
            finishOk();
            return;
    ...
    void loadMoyasarFormScript()
      .then(() => {
        ...
        Moyasar.init({
          ...
          callback_url: buildCafeLiveCallbackUrl(token),
          ...
          on_completed: (payment: unknown) => {
            ...
            activateOnceRef.current = true;
            setActivating(true);
            void activateCafeLive(token, id).then((result) => {
              if (result.ok) {
                window.location.replace(cafeLiveViewHref(token));
                return;
              }
  ...
  return (
    <div dir="rtl" className="min-h-[100svh] bg-[#061018] text-[#f4efe4]">
      <main className="mx-auto max-w-lg px-4 py-12">
        <p className="text-sm font-bold text-[#c48a4a]">halaqmap</p>
        <h1 className="mt-2 text-2xl font-extrabold">دفع كافينا1</h1>
        {!STORE_CAFE_LIVE_CHECKOUT_ENABLED ? (
        ...
        {status === 'live' ? (
          <p className="mt-6 text-sm text-white/70">
            التشغيل حيّ.{' '}
            <a className="underline" href={cafeLiveViewHref(token)}>
              فتح صفحة المقهى
            </a>
          </p>
        ) : null}
        {payable && STORE_CAFE_LIVE_CHECKOUT_ENABLED ? (
          <>
            <p className="mt-3 text-sm leading-7 text-white/70">
              {shopName || STORE_CAFE_LIVE.titleAr} — {priceLabel(priceHalalas)}
            </p>
            {!liveMoney ? (
              <p className="mt-3 rounded-xl border border-[#c48a4a]/30 bg-[#c48a4a]/10 px-3 py-2 text-xs leading-6">
            ...
            {hostedInvoice && !activating ? (
              <a
                href={invoiceUrl}
                className="mt-6 inline-flex w-full items-center justify-center rounded-xl bg-[#c48a4a] px-4 py-3 text-sm font-bold text-[#061018]"
              >
        ...
        <Link to={ROUTE_PATHS.STORE_CAFE} className="mt-8 inline-block text-sm text-white/50 underline">
          {STORE_CAFE_LIVE.titleAr}
        </Link>
      </main>
    </div>
  );
