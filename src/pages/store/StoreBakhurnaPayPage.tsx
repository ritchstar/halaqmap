 ...
 *
 * دفع بخورنا1 — 899 أو 1799 ر.س، وسم store_bakhurna_live.
 */
...
import {
  ...
  STORE_BAKHURNA_LIVE_PRICE_12_HALALAS,
...
import {
  buildBakhurnaLiveCallbackUrl,
  isAllowedMoyasarInvoiceUrl,
  isBakhurnaLivePaymentReturn,
  bakhurnaLiveLivePaymentsEnabled,
  readBakhurnaLiveReturnPaymentId,
  resolveBakhurnaLivePublishableKey,
} from '@/lib/storeBakhurnaLiveMoyasar';
import { activateBakhurnaLive, fetchBakhurnaLivePay, syncBakhurnaLive } from '@/lib/storeBakhurnaLiveRemote';
import { bakhurnaLiveViewHref } from '@/lib/storeHostRedirect';
...
function priceLabel(halalas: number): string {
  if (halalas === STORE_BAKHURNA_LIVE_PRICE_12_HALALAS) return '1799 ر.س لثلاثمئة وستين يوماً';
  if (halalas === STORE_BAKHURNA_LIVE_PRICE_6_HALALAS) return '899 ر.س لمئة وثمانين يوماً';
  return '';
}

function invoiceDescription(halalas: number): string {
  return halalas === STORE_BAKHURNA_LIVE_PRICE_12_HALALAS
    ? 'halaqmap — بخورنا1 360 يوماً'
    : 'halaqmap — بخورنا1 180 يوماً';
}

export default function StoreBakhurnaPayPage() {
  useDocumentTitle('دفع بخورنا1 — halaqmap');
  ...
  const publishableKey = useMemo(() => resolveBakhurnaLivePublishableKey(), []);
  const liveMoney = bakhurnaLiveLivePaymentsEnabled();
  const amountOk =
  ...
  useEffect(() => {
    let cancelled = false;
    void fetchBakhurnaLivePay(token).then((result) => {
      if (cancelled) return;
      if (!result.ok) {
        setStatus('missing');
  ...
  useEffect(() => {
    ...
    if (!isBakhurnaLivePaymentReturn()) return;
    const paymentId = readBakhurnaLiveReturnPaymentId();
    const hasInvoice = isAllowedMoyasarInvoiceUrl(invoiceUrl);
    if (!paymentId && !hasInvoice) return;
    let cancelled = false;
    setActivating(true);
    const run = paymentId ? activateBakhurnaLive(token, paymentId) : syncBakhurnaLive(token);
    void run.then((result) => {
      ...
      const finishOk = (confirmedPaymentId?: string) => {
        ...
        completeStoreLivePaymentAndGo({
          ...
          href: bakhurnaLiveViewHref(token),
        });
      };
      if (result.ok) {
      ...
      if (paymentId && hasInvoice) {
        void syncBakhurnaLive(token).then((synced) => {
          if (cancelled) return;
          if (synced.ok) {
            finishOk();
            return;
    ...
    void loadMoyasarFormScript()
      .then(() => {
        ...
        Moyasar.init({
          ...
          callback_url: buildBakhurnaLiveCallbackUrl(token),
          ...
          on_completed: (payment: unknown) => {
            ...
            activateOnceRef.current = true;
            setActivating(true);
            void activateBakhurnaLive(token, id).then((result) => {
              if (result.ok) {
                completeStoreLivePaymentAndGo({
                  paymentId: id,
                  priceHalalas,
  ...
  return (
    <div dir="rtl" className="min-h-[100svh] bg-[#061018] text-[#f4efe4]">
      <main className="mx-auto max-w-lg px-4 py-12">
        <p className="text-sm font-bold text-[#6E4A26]">halaqmap</p>
        <h1 className="mt-2 text-2xl font-extrabold">دفع بخورنا1</h1>
        {!STORE_BAKHURNA_LIVE_CHECKOUT_ENABLED ? (
          <p className="mt-3 text-sm leading-7 text-white/70">{STORE_BAKHURNA_LIVE.checkoutClosedAr}</p>
        ) : null}
        ...
        {status === 'live' ? (
          <p className="mt-6 text-sm leading-7 text-white/70">
            التشغيل حيّ.{' '}
            <a className="underline" href={bakhurnaLiveViewHref(token)}>
              فتح صفحة النشاط
            </a>
          </p>
        ) : null}
        {payable && STORE_BAKHURNA_LIVE_CHECKOUT_ENABLED ? (
          <>
            <p className="mt-3 text-sm leading-7 text-white/70">
              {shopName || STORE_BAKHURNA_LIVE.titleAr} — {priceLabel(priceHalalas)}
            </p>
            {!liveMoney ? (
              <p className="mt-3 rounded-xl border border-[#6E4A26]/30 bg-[#6E4A26]/10 px-3 py-2 text-xs leading-6">
            ...
            {hostedInvoice && !activating ? (
              <a
                href={invoiceUrl}
                className="mt-6 inline-flex w-full items-center justify-center rounded-xl bg-[#6E4A26] px-4 py-3 text-sm font-bold text-[#061018]"
              >
        ...
        <Link to={ROUTE_PATHS.STORE_BAKHURNA} className="mt-8 inline-block text-sm text-white/50 underline">
          {STORE_BAKHURNA_LIVE.titleAr}
        </Link>
      </main>
    </div>
  );
