...
export const MEDIA_SPOKESPERSON_FORBIDDEN_AR: readonly string[] = [
  ...
  'ممنوع ذكر تاريخ انتهاء أو «ساري حتى» أو «صلاحية الشهادة» لتوثيق التجارة الإلكترونية.',
...
export function resolveMediaSpokespersonSpecialistReferral(
  ...
  if (/(?:كوافير\s*ماب|صالون\s*نسائي|مشغل\s*(?:تجميل|نسائي)|كوافير(?:ة)?|مستقلات)/u.test(m)) {
    return buildGeneralThenReferralReply(
      'كوافير ماب سطح قطاعي نسائي تحت مظلة حلاق ماب — ليست علامة منافسة.',
      'لشرح الفكرة والاستجابة الذكية راجعي **ود** في `/#/coiffeur/interest`. لا عقود من هناك. مسار تسجيل المنشأة لم يُفتح للتعاقد بعد.',
    );
  }
  ...
  if (/(?:استرداد|refund|فشل\s*سداد|moyasar|ميسر|عملية\s*مالية|تحويل\s*بنكي)/iu.test(m)) {
    return buildGeneralThenReferralReply(
      'المدفوعات تتم عبر بوابة الدفع المعتمدة — الحزم مسبقة الدفع ولا تُسترد بعد التفعيل إلا في خلل تقني مثبت.',
...
- للإطار النظامي **الموثّق** (ISIC · توثيق التجارة · تراخيص الإعلام) → استخدم المنهجية أعلاه — **لا تُحِل**
- ضريبة/ZATCA → خبير ZATCA + الإدارة
- خصوصية/قانون تفصيلي → الناظر القانوني
- مدفوعات/استرداد → سياسة بوابة الدفع + \`${PLATFORM_MANAGEMENT_EMAIL}\` برقم الطلب
