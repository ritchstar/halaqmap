...
export const PARTNER_PLATFORM_RELEASE_NOTES: readonly PartnerReleaseNote[] = [
  ...
  },
  {
    ...
    date: '2026-06-10',
    titleAr: 'الرقابة الإدارية — Owner Watch (ذهبي/ماسي)',
    summaryAr:
      'طبقة إشراف منفصلة لإدارة المنشأة: متابعة تشغيل الصالون قراءة فقط من الجوال — حالة المحل، نشاط الشات (عدّ فقط)، تنبيهات، ونبض تشغيلي في الماسي — دون قراءة نصوص الزبائن.',
    topics: [
      'الرقابة الإدارية',
      'رقابة إدارية',
      'Owner Watch',
    ...
    bulletsAr: [
      ...
      OWNER_WATCH_FEATURE_GOLD_LINE_AR,
      OWNER_WATCH_FEATURE_DIAMOND_LINE_AR,
      'البرونزي: لا تتضمّن رقابة إدارية — للتشغيل اليومي استخدم لوحة التحكم فقط.',
      'حساب الإدارة = نفس بريد تفعيل الرخصة؛ يصله رابط magic لمرة واحدة + رابط دائم للمفضلة في بريد التفعيل (ذهبي/ماسي).',
  ...
  },
  {
    ...
    bulletsAr: [
      ...
      'ما يتغيّر عملياً للحلاق: لا تغيير في السعر (برونزي 200 · ذهبي 300 · ماسي 400) — يتغيّر فقط منطق الظهور: استجابة برمجية عند الطلب بدل قائمة دائمة، مع أولويات استجابة مختلفة لكل مستوى.',
  ...
  },
  {
    ...
    summaryAr:
      `«المناوب الرقمي الذكي 🌙» لم يعد مدمجاً ضمن الماسية؛ هو Add-on اختياري +${DIGITAL_SHIFT_MONTHLY_ADDON_SAR} ر.س/حزمة رخصة يعزّز الرخصة التقنية للماسية.`,
    topics: ['المناوب', 'المناوب الرقمي', 'ماسي', 'Add-on', '450', '50'],
    bulletsAr: [
      `${DIAMOND_PRODUCT_STANDARD_LABEL_AR}: ${TIER_MONTHLY_SAR.diamond} ر.س/حزمة رخصة (30 يوم إدراج).`,
      `${DIAMOND_PRODUCT_SMART_LABEL_AR}: ${DIAMOND_WITH_ADDON_MONTHLY_SAR} ر.س/حزمة (= ${TIER_MONTHLY_SAR.diamond} + ${DIGITAL_SHIFT_MONTHLY_ADDON_SAR} Add-on).`,
...
function composePartnerProductFactsPack(): string {
  return [
    ...
    `  · برونزي: ${TIER_MONTHLY_SAR.bronze} ر.س/حزمة`,
    `  · ذهبي: ${TIER_MONTHLY_SAR.gold} ر.س/حزمة`,
    `  · ماسي (رخصة نفاذ): ${TIER_MONTHLY_SAR.diamond} ر.س/حزمة`,
    `  · ${DIGITAL_SHIFT_PRICING_ADDON_LABEL_AR} — للماسية فقط (اختياري).`,
    `  · ${DIAMOND_PRODUCT_SMART_LABEL_AR}: **${DIAMOND_WITH_ADDON_MONTHLY_SAR} ر.س/حزمة** (= ${TIER_MONTHLY_SAR.diamond} + ${DIGITAL_SHIFT_MONTHLY_ADDON_SAR}).`,
    '',
    ...
    `- **الذهبي — الرقابة الإدارية:** ${OWNER_WATCH_FEATURE_GOLD_LINE_AR}`,
    ...
    `- **الماسي — الرقابة الإدارية:** ${OWNER_WATCH_FEATURE_DIAMOND_LINE_AR}`,
    ...
    `- **السعر:** +${DIGITAL_SHIFT_MONTHLY_ADDON_SAR} ر.س لكل حزمة رخصة نفاذ ماسية (منفصل عن ${TIER_MONTHLY_SAR.diamond} ر.س).`,
