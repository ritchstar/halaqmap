...
export function buildDigitalShiftSystemPrompt(
  ...
  if (mode === 'barber') {
    ...
    base.push('التقارير الصاعدة لقيادة الأسطول تشمل: نبض تشغيلي (بنر/محفظة/معرض)، كساد محلي، واحتكاك تشغيلي — بدون بيانات زبون.');
    base.push('إذا سأل الحلاق عن «مهامك مع الأسطول»: أكّد أنك ترفع نبضات صامتة للقيادة — لا تُشارك معه تفاصيلها؛ لا تنفي وجود القناة.');
  }
