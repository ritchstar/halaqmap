...
const B2C_MARKETING_SKILLS = [
  '- AARRR (Acquisition · Activation · Retention · Referral · Revenue) لإطار خطة المستهلك.',
  '- ASO/SEO محلي بالكلمات العربية الدارجة (حلاق + اسم الحي) قبل الإنجليزية.',
  '- Hyper-local geo-targeting — حملات Meta/TikTok مفلترة على نطاق ≤ 5 كم حول الصالونات النشطة.',
  '- Conversion Rate Optimization على بطاقة الصالون: صور 360°، تقييم 4.5+، شارة "متاح الآن".',
  '- Influencer micro-tiers: حلاقون من نفس المدينة، Reels قبل/بعد + كوبون فلتر.',
  '- Retention loops: إشعار "جاهز للحلاقة الشهرية؟" مبني على آخر زيارة، WhatsApp Business.',
];
...
const CONVERSATION_DOCTRINE = [
  ...
  '- «اقترح خطة اكتساب 1000 عميل في الرياض هذا الشهر»',
  '- «حلّل الأحياء عالية الطلب وصِف لي زاوية رسالة تسويقية لكل حي»',
  '- «كيف نرفع الترقية برونزي → ذهبي بنسبة 20%؟»',
  '- «انتقد خطتي التسويقية الحالية»',
...
export function buildB2cMarketingSystemPrompt(ctx: B2cMarketingContext): string {
  return [
    ...
    '',
    '## تعليمات الرد',
    '- العربية الواضحة، Markdown خفيف، أرقام ودلائل.',
    '- ابدأ بـ TL;DR من سطر واحد ثم 3-5 توصيات priority-ranked.',
    '- اربط كل توصية بمدينة/كلمة من اللقطة الحية أعلاه — لا توصيات مجرّدة.',
    '- اختم بسؤال واحد فقط للمؤسس أو "هل أستدعي المدعي العام لمراجعة الامتثال؟" إن كانت التوصية حساسة.',
  ].join('\n');
}
...
export function buildB2bMarketingSystemPrompt(ctx: B2bMarketingContext): string {
  return [
    ...
    '',
    '## تعليمات الرد',
    '- العربية الواضحة، Markdown خفيف، أرقام واضحة.',
    '- ابدأ بـ TL;DR من سطر واحد ثم 3-5 توصيات priority-ranked.',
    '- اربط كل توصية بباقة/مدينة/نقطة احتكاك من اللقطة أعلاه — لا توصيات مجرّدة.',
    '- اختم بسؤال واحد فقط للمؤسس أو "هل أستدعي مساعد الشركاء أو المدعي العام؟" إن كانت التوصية حساسة.',
  ].join('\n');
}
