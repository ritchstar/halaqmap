 ...
 *
 * تحصيل بخورنا1 — وسم store_bakhurna_live، 899 أو 1799 ر.س. نافذة الاستفسار مدرجة.
 */
...
import {
  bakhurnaChargeHalalas,
  bakhurnaLiveInvoiceDescription,
  bakhurnaLiveInvoiceMetadata,
  bakhurnaLiveIsExpired,
...
} from './_lib/storeBakhurnaLive.js';
...
function successUrl(token: string, request: Request): string {
  ...
  q.set('purpose', STORE_BAKHURNA_LIVE_PRODUCT);
  q.set('store_bakhurna_token', token);
...
type BakhurnaRow = {
  ...
  payload: BakhurnaLiveOrderPayload & Record<string, unknown>;
  expires_at: string | null;
};

function expiredPayload(row: BakhurnaRow) {
  return {
    ok: true,
    expired: true,
    renewToken: row.shop_token,
    landingPath: `/store/bakhurna?renew=${encodeURIComponent(row.shop_token)}`,
    shopName: String(row.payload?.shopName || ''),
    expiresAt: row.expires_at,
  };
}

function isTermExpired(row: Pick<BakhurnaRow, 'status' | 'expires_at'>): boolean {
  if (row.status === 'revoked') return true;
  if (row.status === 'expired' || row.status === 'pending_renewal') return true;
  if (row.status === 'live' && bakhurnaLiveIsExpired(row.expires_at)) return true;
  return false;
}
...
export async function GET(request: Request): Promise<Response> {
  ...
  if (!token) {
    return json({ ok: true, route: 'public-store-bakhurna-live', product: STORE_BAKHURNA_LIVE_PRODUCT }, 200, headers);
  }
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (blocked) return blocked;
  const headers = corsHeaders(request);
  const guard = runRegistrationRouteGuards(request, 'public-store-bakhurna-live');
...
async function findByAnyToken(db: Db, token: string): Promise<BakhurnaRow | null> {
  for (const col of ['shop_token', 'desk_token'] as const) {
    const { data } = await db.from(STORE_BAKHURNA_LIVE_TABLE).select('*').eq(col, token).maybeSingle();
    if (data) return data as BakhurnaRow;
  }
  return null;
}
...
async function readRow(db: Db, row: BakhurnaRow, role: string, headers: Record<string, string>) {
  const payload = (row.payload || {}) as BakhurnaLiveOrderPayload;
  if (role === 'pay') {
    return json(
      {
        ok: true,
  ...
  const clock = await applyStoreTrialClock(db, row, STORE_BAKHURNA_LIVE_TABLE);
  if (clock.expired || isTermExpired(row)) return json(expiredPayload(row), 200, headers);
  if (row.status !== 'live') return json({ error: 'التشغيل لم يُفعَّل بعد' }, 403, headers);
  return json(
    {
      ok: true,
      status: row.status,
      role,
      payload: (() => {
        const built = publicBakhurnaPayload(payload, role);
...
async function attachInvoice(
  ...
  const created = await createMoyasarInvoice(secret, {
    amount: bakhurnaChargeHalalas(packId),
    currency: 'SAR',
    description: bakhurnaLiveInvoiceDescription(packId),
    success_url: successUrl(shopToken, request),
    back_url: `${storeOrigin()}/#/store/bakhurna`,
    callback_url: `${payOrigin(request)}/api/public-store-bakhurna-live`,
    expired_at: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
    metadata: bakhurnaLiveInvoiceMetadata(shopToken, packId, kind, affiliateCode, vendorMode),
  });
...
async function createPending(db: Db, body: Record<string, unknown>, headers: Record<string, string>, request: Request) {
  if (!isBakhurnaLiveCheckoutEnabled()) {
    return json({ error: 'تحصيل بخورنا1 مغلق حالياً.' }, 503, headers);
  }
  const renewToken = String(body.renewToken || '').trim();
  if (renewToken) return createRenewal(db, body, headers, request);
  const parsedRaw = parseBakhurnaLiveOrderBody(body);
  ...
  const charge = bakhurnaChargeHalalas(parsed.packId);
  const shopToken = newBakhurnaToken();
  const deskToken = newBakhurnaToken();
  const { error } = await db.from(STORE_BAKHURNA_LIVE_TABLE).insert({
  ...
  return json(
    {
      ...
      deskToken,
      priceHalalas: charge,
      payPath: `/pay/bakhurna/${shopToken}`,
      invoiceUrl,
...
async function createRenewal(
  ...
  const charge = bakhurnaChargeHalalas(packId);
    ...
    return json(
      {
        ...
        priceHalalas: row.price_halalas,
        payPath: `/pay/bakhurna/${row.shop_token}`,
        invoiceUrl: invoiceUrlFromPayload(row.payload),
        shopUrl: shopUrl(row.shop_token),
        deskUrl: deskUrl(row.desk_token),
      },
  ...
  if (row.status === 'live' && !bakhurnaLiveIsExpired(row.expires_at)) {
...
async function markLive(db: Db, id: string, paymentId: string, amount: number): Promise<boolean> {
  ...
  if (
    ...
    !bakhurnaLiveIsExpired(current.expires_at)
  ) {
    return true;
  }
  const wasRenewal = current.status === 'pending_renewal' || current.status === 'expired' || current.status === 'live';
  const pack = bakhurnaPackFromHalalas(amount);
  const expiresAt = bakhurnaLiveTermEndIso(pack.days);
  ...
  if (updated) {
    ...
    void sendBakhurnaLiveLinksEmail({
  ...
  const { data: again } = await db.from(STORE_BAKHURNA_LIVE_TABLE).select('status').eq('id', id).maybeSingle();
...
async function fulfillFromPaymentId(db: Db, token: string, paymentId: string, headers: Record<string, string>) {
  const { data } = await db.from(STORE_BAKHURNA_LIVE_TABLE).select('*').eq('shop_token', token).maybeSingle();
  if (!data) return json({ error: 'الطلب غير موجود' }, 404, headers);
  const row = data as BakhurnaRow;
  if (row.status === 'live' && !bakhurnaLiveIsExpired(row.expires_at) && String(row.moyasar_payment_id || '') === paymentId) {
  ...
  if (!isBakhurnaPriceHalalas(Number(parsed.amount)) || (Number(row.price_halalas) > 0 && Number(parsed.amount) !== Number(row.price_halalas))) {
    return json({ error: 'مبلغ الدفع لا يطابق باقة بخورنا1' }, 409, headers);
  }
  const metaOk = bakhurnaLivePaymentMatches({ meta: parsed.metadata, token, amount: Number(parsed.amount) });
  if (!metaOk) {
    const invoiceId = invoiceIdFromPayload(row.payload) || String(parsed.invoice_id || '').trim();
    if (!invoiceId) return json({ error: 'وسم الدفع لا يطابق بخورنا1' }, 409, headers);
    ...
    try {
      ...
      if (!bakhurnaLivePaymentMatches({ meta: inv.metadata, token, amount: Number(inv.amount) })) {
        return json({ error: 'وسم الفاتورة لا يطابق بخورنا1' }, 409, headers);
      }
    } catch {
...
async function activatePaid(db: Db, body: Record<string, unknown>, headers: Record<string, string>) {
  if (!isBakhurnaLiveCheckoutEnabled()) return json({ error: 'تحصيل بخورنا1 مغلق حالياً.' }, 503, headers);
...
async function syncPaid(db: Db, body: Record<string, unknown>, headers: Record<string, string>) {
  ...
  try {
    ...
    if (Number(parsed.amount) !== row.price_halalas || !isBakhurnaPriceHalalas(Number(parsed.amount))) {
      return json({ error: 'مبلغ الفاتورة لا يطابق باقة بخورنا1' }, 409, headers);
    }
    if (!bakhurnaLivePaymentMatches({ meta: parsed.metadata, token, amount: Number(parsed.amount) })) {
      return json({ error: 'وسم الفاتورة لا يطابق بخورنا1' }, 409, headers);
    }
...
async function addChat(db: Db, body: Record<string, unknown>, headers: Record<string, string>) {
  const token = String(body.token || '').trim();
  const chat = parseBakhurnaChat(body.chat, 'buyer');
  ...
  if (row.status !== 'live' || isTermExpired(row)) return json({ error: 'انتهت مدة التشغيل' }, 403, headers);
  const payload = { ...(row.payload as BakhurnaLiveOrderPayload) };
  const chats = parseBakhurnaChats(payload.chats);
  chats.unshift(chat);
...
async function saveHost(db: Db, body: Record<string, unknown>, headers: Record<string, string>) {
  const token = String(body.token || '').trim();
  if (!token) return json({ error: 'رابط لوحة التشغيل غير صالح' }, 400, headers);
  const { data } = await db.from(STORE_BAKHURNA_LIVE_TABLE).select('*').eq('desk_token', token).maybeSingle();
  if (!data) return json({ error: 'رابط لوحة التشغيل غير صالح' }, 404, headers);
  const row = data as BakhurnaRow;
  if (row.status !== 'live' || isTermExpired(row)) return json({ error: 'انتهت مدة التشغيل' }, 403, headers);
  const current = { ...(row.payload as BakhurnaLiveOrderPayload) };
  const next: BakhurnaLiveOrderPayload = {
    ...current,
