...
export async function POST(request: Request): Promise<Response> {
  ...
  const system = [
    ...
    '4) رؤية الطلب: استخدم الحقل searchDemand في JSON فقط إذا كان موجوداً ومفعلاً. إذا أظهر الملخص أن سجل البحث الجغرافي معطّل، فاذكر ذلك صراحةً ولا تختلق أي قراءة طلب أو تنبيه استقطاب من بيانات غير متاحة.',
    '',
    '--- ملخص بيانات (JSON) ---',
    briefJson || '{}',
  ].join('\n');
