 ...
 */
/**
 * تأكيد بريد تسجيل مدرسة البلوت عبر رابط الرسالة. مطابق حرفياً لـ
 * chess-school-confirm-email.ts.
 */
import { createClient } from '@supabase/supabase-js';
import { confirmBalootSchoolRegistrationEmail } from './_lib/balootSchoolRegistrationService.js';
...
export async function GET(request: Request): Promise<Response> {
  ...
  const guard = runRegistrationRouteGuards(request, 'baloot-school-confirm-email');
  if (guard.ok === false) {
...
async function handleConfirm(token: string, headers: Record<string, string>): Promise<Response> {
  ...
  const result = await confirmBalootSchoolRegistrationEmail(supabase, token);
  if (!result.ok) {
  ...
  return Response.json(
    {
      ...
      payPath: `/baloot/school/pay/${encodeURIComponent(result.registrationId)}`,
      messageAr:
        'تم تأكيد بريدك بنجاح وتفعيل حسابك. الخطوة الأخيرة هي دفع الاشتراك (199 ريال — دفعة واحدة، وصول دائم) لتفعيل مدرسة البلوت باسمك.',
    },
    { status: 200, headers },
  );
}
