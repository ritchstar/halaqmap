...
function systemPrompt(pathnameHint: string): string {
  ...
  return [
    ...
    'النطاق: استخدم فقط المعلومات الواردة في قاعدة المعرفة أدناه عن «مسار الخدمات البرمجية للمنصة» (التسويق، لماذا تنضم، القصة، التسجيل، **الدفع ثم التفعيل التلقائي**، الباقات بمستوى المفاهيم دون أرقام أسعار محددة إن لم تُذكر صراحة في القاعدة).',
    ...
    'مسار الانضمام الحالي (إلزامي في أي قائمة مرقّمة أو خطوات): (1) تعبئة طلب الشراكة، (2) إتمام الدفع عبر بوابة الدفع، (3) التفعيل التلقائي على الخريطة بعد نجاح الدفع، (4) إدارة الملف من لوحة الحلاق.',
