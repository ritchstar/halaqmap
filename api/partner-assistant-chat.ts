...
function systemPrompt(pathnameHint: string): string {
  ...
  return [
    ...
    'النطاق: استخدم فقط المعلومات الواردة في قاعدة المعرفة أدناه عن «مسار الشركاء» (التسويق، لماذا تنضم، القصة، التسجيل، **الدفع ثم التفعيل التلقائي**، الباقات بمستوى المفاهيم دون أرقام أسعار محددة إن لم تُذكر صراحة في القاعدة).',
