 ...
 */
/**
 * دفع مدرسة البلوت — 199 ر.س، Moyasar حقيقي مباشر. action: 'get_public'
 * (حالة الدفع للعرض) | 'activate_paid' (تفعيل بعد تحقق حقيقي من ميسر).
 * مطابق حرفياً لـ chess-school-pay.ts.
 */
import { createClient } from '@supabase/supabase-js';
import { activateBalootSchoolPaid, getBalootSchoolPayPublic } from './_lib/balootSchoolPaymentService.js';
...
export async function GET(request: Request): Promise<Response> {
  ...
  return Response.json({ ok: true, route: 'baloot-school-pay' }, { headers: corsHeaders(request) });
}
...
export async function POST(request: Request): Promise<Response> {
  ...
  const guard = runRegistrationRouteGuards(request, 'baloot-school-pay');
  if (guard.ok === false) {
  ...
  if (action === 'get_public') {
    const result = await getBalootSchoolPayPublic(db, String(body.registrationId || ''));
    if (!result.ok) return json({ ok: false, error: result.error }, result.status, headers);
    return json(result, 200, headers);
  }

  if (action === 'activate_paid') {
    const result = await activateBalootSchoolPaid(db, {
