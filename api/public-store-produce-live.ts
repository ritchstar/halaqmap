 ...
 *
 * تحصيل خضارنا1 — وسم store_produce_live، 1350 أو 2500 ر.س. صندوق الملاحظة مدرج.
 */
...
import {
  ...
  produceLiveInvoiceMetadata,
  produceLiveIsExpired,
  produceLivePaymentMatches,
  produceLiveTermEndIso,
  producePackFromHalalas,
  isProduceLiveCheckoutEnabled,
  isProducePriceHalalas,
  newProduceToken,
  parseProduceChat,
  parseProduceChats,
  parseProduceLiveOrderBody,
  parseProducePackId,
  publicProducePayload,
  STORE_PRODUCE_LIVE_POLICY,
...
function successUrl(token: string, request: Request): string {
  ...
  q.set('purpose', STORE_PRODUCE_LIVE_PRODUCT);
  q.set('store_produce_token', token);
...
type ProduceRow = {
  ...
  payload: ProduceLiveOrderPayload & Record<string, unknown>;
  expires_at: string | null;
};

function expiredPayload(row: ProduceRow) {
  return {
    ok: true,
    expired: true,
    renewToken: row.shop_token,
    landingPath: `/store/produce?renew=${encodeURIComponent(row.shop_token)}`,
    shopName: String(row.payload?.shopName || ''),
    expiresAt: row.expires_at,
  };
}

function isTermExpired(row: Pick<ProduceRow, 'status' | 'expires_at'>): boolean {
  if (row.status === 'revoked') return true;
  if (row.status === 'expired' || row.status === 'pending_renewal') return true;
  if (row.status === 'live' && produceLiveIsExpired(row.expires_at)) return true;
  return false;
}
...
export async function GET(request: Request): Promise<Response> {
  ...
  if (!token) {
    return json({ ok: true, route: 'public-store-produce-live', product: STORE_PRODUCE_LIVE_PRODUCT }, 200, headers);
  }
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (blocked) return blocked;
  const headers = corsHeaders(request);
  const guard = runRegistrationRouteGuards(request, 'public-store-produce-live');
...
async function findByAnyToken(db: Db, token: string): Promise<ProduceRow | null> {
  for (const col of ['shop_token', 'desk_token'] as const) {
    const { data } = await db.from(STORE_PRODUCE_LIVE_TABLE).select('*').eq(col, token).maybeSingle();
    if (data) return data as ProduceRow;
  }
  return null;
}
...
async function readByRole(db: Db, token: string, role: string, headers: Record<string, string>) {
  ...
  return readRow(db, data as ProduceRow, role, headers);
}

async function readRow(db: Db, row: ProduceRow, role: string, headers: Record<string, string>) {
  const payload = (row.payload || {}) as ProduceLiveOrderPayload;
  if (role === 'pay') {
    return json(
      {
        ok: true,
  ...
  const clock = await applyStoreTrialClock(db, row, STORE_PRODUCE_LIVE_TABLE);
  ...
  return json(
    {
      ...
      role,
      payload: (() => {
        const built = publicProducePayload(payload, role);
...
async function attachInvoice(
  ...
  const created = await createMoyasarInvoice(secret, {
    amount: produceChargeHalalas(packId),
    currency: 'SAR',
    description: produceLiveInvoiceDescription(packId),
    success_url: successUrl(shopToken, request),
    back_url: `${storeOrigin()}/#/store/produce`,
    callback_url: `${payOrigin(request)}/api/public-store-produce-live`,
    expired_at: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
    metadata: produceLiveInvoiceMetadata(shopToken, packId, kind, affiliateCode, vendorMode),
  });
...
async function createPending(db: Db, body: Record<string, unknown>, headers: Record<string, string>, request: Request) {
  if (!isProduceLiveCheckoutEnabled()) {
    return json({ error: 'تحصيل خضار الحي مغلق حالياً.' }, 503, headers);
  }
  ...
  const deskToken = newProduceToken();
  const { error } = await db.from(STORE_PRODUCE_LIVE_TABLE).insert({
    ...
    policy_version: STORE_PRODUCE_LIVE_POLICY,
  });
...
async function createRenewal(
  ...
  const charge = produceChargeHalalas(packId);
    ...
    return json(
      {
        ...
        priceHalalas: row.price_halalas,
        payPath: `/pay/produce/${row.shop_token}`,
        invoiceUrl: invoiceUrlFromPayload(row.payload),
        shopUrl: shopUrl(row.shop_token),
        deskUrl: deskUrl(row.desk_token),
      },
  ...
  if (row.status === 'live' && !produceLiveIsExpired(row.expires_at)) {
...
async function markLive(db: Db, id: string, paymentId: string, amount: number): Promise<boolean> {
  ...
  if (
    ...
    !produceLiveIsExpired(current.expires_at)
  ) {
    return true;
  }
  const wasRenewal = current.status === 'pending_renewal' || current.status === 'expired' || current.status === 'live';
  const pack = producePackFromHalalas(amount);
  const expiresAt = produceLiveTermEndIso(pack.days);
  ...
  if (updated) {
    ...
    void sendProduceLiveLinksEmail({
  ...
  const { data: again } = await db.from(STORE_PRODUCE_LIVE_TABLE).select('status').eq('id', id).maybeSingle();
...
async function fulfillFromPaymentId(db: Db, token: string, paymentId: string, headers: Record<string, string>) {
  const { data } = await db.from(STORE_PRODUCE_LIVE_TABLE).select('*').eq('shop_token', token).maybeSingle();
  if (!data) return json({ error: 'الطلب غير موجود' }, 404, headers);
  const row = data as ProduceRow;
  if (row.status === 'live' && !produceLiveIsExpired(row.expires_at) && String(row.moyasar_payment_id || '') === paymentId) {
  ...
  if (!isProducePriceHalalas(Number(parsed.amount)) || (Number(row.price_halalas) > 0 && Number(parsed.amount) !== Number(row.price_halalas))) {
    return json({ error: 'مبلغ الدفع لا يطابق باقة خضار الحي' }, 409, headers);
  }
  const metaOk = produceLivePaymentMatches({ meta: parsed.metadata, token, amount: Number(parsed.amount) });
  if (!metaOk) {
    const invoiceId = invoiceIdFromPayload(row.payload) || String(parsed.invoice_id || '').trim();
    if (!invoiceId) return json({ error: 'وسم الدفع لا يطابق خضار الحي' }, 409, headers);
    ...
    try {
      ...
      if (!produceLivePaymentMatches({ meta: inv.metadata, token, amount: Number(inv.amount) })) {
        return json({ error: 'وسم الفاتورة لا يطابق خضار الحي' }, 409, headers);
      }
    } catch {
...
async function activatePaid(db: Db, body: Record<string, unknown>, headers: Record<string, string>) {
  if (!isProduceLiveCheckoutEnabled()) return json({ error: 'تحصيل خضار الحي مغلق حالياً.' }, 503, headers);
...
async function syncPaid(db: Db, body: Record<string, unknown>, headers: Record<string, string>) {
  ...
  try {
    ...
    if (Number(parsed.amount) !== row.price_halalas || !isProducePriceHalalas(Number(parsed.amount))) {
      return json({ error: 'مبلغ الفاتورة لا يطابق باقة خضار الحي' }, 409, headers);
    }
    if (!produceLivePaymentMatches({ meta: parsed.metadata, token, amount: Number(parsed.amount) })) {
      return json({ error: 'وسم الفاتورة لا يطابق خضار الحي' }, 409, headers);
    }
...
async function addChat(db: Db, body: Record<string, unknown>, headers: Record<string, string>) {
  const token = String(body.token || '').trim();
  const chat = parseProduceChat(body.chat, 'buyer');
  ...
  if (row.status !== 'live' || isTermExpired(row)) return json({ error: 'انتهت مدة التشغيل' }, 403, headers);
  const payload = { ...(row.payload as ProduceLiveOrderPayload) };
  const chats = parseProduceChats(payload.chats);
  chats.unshift(chat);
...
async function saveHost(db: Db, body: Record<string, unknown>, headers: Record<string, string>) {
  const token = String(body.token || '').trim();
  if (!token) return json({ error: 'رابط الصندوق غير صالح' }, 400, headers);
  const { data } = await db.from(STORE_PRODUCE_LIVE_TABLE).select('*').eq('desk_token', token).maybeSingle();
  if (!data) return json({ error: 'رابط الصندوق غير صالح' }, 404, headers);
  const row = data as ProduceRow;
  if (row.status !== 'live' || isTermExpired(row)) return json({ error: 'انتهت مدة التشغيل' }, 403, headers);
  const current = { ...(row.payload as ProduceLiveOrderPayload) };
  const next: ProduceLiveOrderPayload = {
    ...
    chatIncluded: true,
    chats: Array.isArray(body.chats) ? parseProduceChats(body.chats) : parseProduceChats(current.chats),
