...
async function readByRole(db: Db, token: string, role: string, headers: Record<string, string>) {
  ...
  return readRow(data as RestaurantRow, role, headers);
}

function readRow(row: RestaurantRow, role: string, headers: Record<string, string>) {
...
async function createRenewal(
  ...
  const pack = restaurantPackFromHalalas(row.price_halalas);
...
async function fulfillFromPaymentId(db: Db, token: string, paymentId: string, headers: Record<string, string>) {
  ...
  if (!isRestaurantPriceHalalas(Number(parsed.amount)) || Number(parsed.amount) !== row.price_halalas) {
