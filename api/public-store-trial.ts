 ...
 *
 * طلب التجربة العامة وتأكيد البريد. لا كاردي8 ولا قاعات المناسبة.
 */
...
export async function GET(request: Request): Promise<Response> {
  ...
  if (!token) return json({ ok: true }, 200, headers);
  const confirmed = await confirmGeneralTrial(db, token);
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (blocked) return blocked;
  const headers = corsHeaders(request);
  const guard = runRegistrationRouteGuards(request, 'public-store-trial');
  ...
  if (String(body.website ?? body.company_url_hp ?? '').trim()) {
    await recordHoneypotTrip(request, 'public-store-trial');
  ...
  const mailed = await sendGeneralTrialConfirmEmail({
    to: String(body.email || '').trim().toLowerCase(),
    confirmUrl: generalTrialConfirmUrl(entered.confirmToken),
  });
  if (!mailed) {
