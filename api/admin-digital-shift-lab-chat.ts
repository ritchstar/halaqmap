...
import {
  buildDigitalShiftAdminLabSystemPrompt,
...
function parseHistory(raw: unknown): DigitalShiftLabChatTurn[] {
  ...
  return raw
    ...
    .filter((x): x is DigitalShiftLabChatTurn => x !== null)
    .slice(-10);
}
...
export async function GET(request: Request): Promise<Response> {
  ...
  return json({
    ok: true,
    route: 'admin-digital-shift-lab-chat',
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!userMessage && !imageBase64Raw) {
    return json({ error: 'أدخل رسالة أو ارفع صورة للاختبار' }, 400);
  }
  ...
  const snapshot = await loadDigitalShiftFleetSnapshot(auth.supabase);
