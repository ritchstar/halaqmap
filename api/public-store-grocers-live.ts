...
type GrocersRow = {
  ...
  payload: GrocersLiveOrderPayload & Record<string, unknown>;
  expires_at: string | null;
};

function expiredPayload(row: GrocersRow) {
  return {
    ok: true,
    expired: true,
    renewToken: row.shop_token,
    landingPath: `/store/grocers?renew=${encodeURIComponent(row.shop_token)}`,
    shopName: String(row.payload?.shopName || ''),
    expiresAt: row.expires_at,
  };
}

function isTermExpired(row: Pick<GrocersRow, 'status' | 'expires_at'>): boolean {
  if (row.status === 'revoked') return true;
  if (row.status === 'expired' || row.status === 'pending_renewal') return true;
  if (row.status === 'live' && grocersLiveIsExpired(row.expires_at)) return true;
  return false;
}
...
export async function GET(request: Request): Promise<Response> {
  ...
  if (!token) {
    return json({ ok: true, route: 'public-store-grocers-live', product: STORE_GROCERS_LIVE_PRODUCT }, 200, headers);
  }
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (blocked) return blocked;
  const headers = corsHeaders(request);
  const guard = runRegistrationRouteGuards(request, 'public-store-grocers-live');
...
async function findByAnyToken(db: Db, token: string): Promise<GrocersRow | null> {
  for (const col of ['shop_token', 'desk_token'] as const) {
    const { data } = await db.from(STORE_GROCERS_LIVE_TABLE).select('*').eq(col, token).maybeSingle();
    if (data) return data as GrocersRow;
  }
  return null;
}
...
async function createPending(db: Db, body: Record<string, unknown>, headers: Record<string, string>, request: Request) {
  if (!isGrocersLiveCheckoutEnabled()) {
  ...
  const parsedRaw = parseGrocersLiveOrderBody(body);
...
async function markLive(db: Db, id: string, paymentId: string, amount: number): Promise<boolean> {
  ...
  if (
    ...
    !grocersLiveIsExpired(current.expires_at)
  ) {
    return true;
  }
  ...
  const payload: Record<string, unknown> = {
    ...
    packId: pack.id,
    chatAddon: grocersChatAddonFromHalalas(amount),
  };
  ...
  if (updated) {
    ...
    void sendGrocersLiveLinksEmail({
...
async function fulfillFromPaymentId(db: Db, token: string, paymentId: string, headers: Record<string, string>) {
  ...
  if (row.status === 'live' && !grocersLiveIsExpired(row.expires_at) && String(row.moyasar_payment_id || '') === paymentId) {
  ...
  if (!metaOk) {
    ...
    if (!invoiceId) return json({ error: 'وسم الدفع لا يطابق تموينات الحي' }, 409, headers);
    ...
    try {
      ...
      if (!grocersLivePaymentMatches({ meta: inv.metadata, token, amount: Number(inv.amount) })) {
        return json({ error: 'وسم الفاتورة لا يطابق تموينات الحي' }, 409, headers);
      }
    } catch {
...
async function activatePaid(db: Db, body: Record<string, unknown>, headers: Record<string, string>) {
  if (!isGrocersLiveCheckoutEnabled()) return json({ error: 'تحصيل تموينات الحي مغلق حالياً.' }, 503, headers);
...
async function syncPaid(db: Db, body: Record<string, unknown>, headers: Record<string, string>) {
  ...
  try {
    ...
    if (Number(parsed.amount) !== row.price_halalas || !isGrocersPriceHalalas(Number(parsed.amount))) {
      return json({ error: 'مبلغ الفاتورة لا يطابق باقة تموينات الحي' }, 409, headers);
    }
    if (!grocersLivePaymentMatches({ meta: parsed.metadata, token, amount: Number(parsed.amount) })) {
      return json({ error: 'وسم الفاتورة لا يطابق تموينات الحي' }, 409, headers);
    }
...
async function saveHost(db: Db, body: Record<string, unknown>, headers: Record<string, string>) {
  ...
  const { data } = await db.from(STORE_GROCERS_LIVE_TABLE).select('*').eq('desk_token', token).maybeSingle();
  if (!data) return json({ error: 'رابط الكاشير غير صالح' }, 404, headers);
  const row = data as GrocersRow;
  if (row.status !== 'live' || isTermExpired(row)) return json({ error: 'انتهت مدة التشغيل' }, 403, headers);
  const current = { ...(row.payload as GrocersLiveOrderPayload) };
