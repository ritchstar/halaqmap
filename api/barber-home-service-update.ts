...
const HOME_VISIT_CATEGORY = 'زيارة منزلية';
const HOME_VISIT_ALIASES = new Set(['زيارة منزلية', 'خدمة منزلية']);
...
function withHomeVisitInSpecialties(specialties: unknown, offered: boolean): string[] {
  const base = Array.isArray(specialties)
    ? specialties.map((s) => String(s).trim()).filter(Boolean)
    : [];
  const without = base.filter((s) => !HOME_VISIT_ALIASES.has(s));
  if (!offered) return without;
  if (without.includes(HOME_VISIT_CATEGORY)) return without;
  return [...without, HOME_VISIT_CATEGORY];
}
...
export async function GET(request: Request): Promise<Response> {
  ...
  return Response.json(
    {
      ok: true,
      route: 'barber-home-service-update',
...
export async function POST(request: Request): Promise<Response> {
  ...
  const guard = runRegistrationRouteGuards(request, 'barber-portal-home-service-update');
  if (guard.ok === false) {
  ...
  const radiusRaw = (body as { radiusKm?: unknown }).radiusKm;
  ...
  if (tier !== 'gold' && tier !== 'diamond') {
    return Response.json(
      { error: 'Home visit service controls are available for gold and diamond partners only.' },
      { status: 403, headers },
    );
  }
  ...
  if (offered) {
    ...
    if (!Number.isFinite(p) || p <= 0) {
      return Response.json(
        { error: 'When home visit is enabled, a displayed price greater than zero is required.' },
        { status: 400, headers },
      );
    }
    priceSar = Math.round(p * 100) / 100;
    const r = radiusRaw == null || radiusRaw === '' ? NaN : Number(radiusRaw);
    if (Number.isFinite(r) && r > 0) {
      radiusKm = Math.min(100, Math.max(1, Math.floor(r)));
    }
  }

  const nextSpecialties = withHomeVisitInSpecialties(b.specialties, offered);
  const updatePayload: Record<string, unknown> = {
    home_service_offered: offered,
    home_service_price_sar: offered ? priceSar : null,
    home_service_radius_km: offered ? radiusKm : null,
    home_service_public_visible: offered ? publicVisible : true,
    home_service_customer_note: offered && customerNote ? customerNote : null,
  ...
  if (upErr) {
    if (/home_service/i.test(upErr.message || '')) {
      return Response.json(
        {
          error: 'home_service_columns_missing',
          hint: 'Apply migration 109_barber_home_visit_service.sql on Supabase.',
        },
        { status: 503, headers },
      );
    }
