 ...
 *
 * مشاركة هدية خريطة الحل وتأكيد البريد. لا كاردي8.
 */
...
import {
  ...
  giftConfirmUrl,
  publicGiftState,
} from './_lib/storeGiftCampaign.js';
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (blocked) return blocked;
  const headers = corsHeaders(request);
  const guard = runRegistrationRouteGuards(request, 'public-store-gift');
  ...
  if (action === 'confirm') {
    const confirmed = await confirmGiftEntry(db, String(body.token || ''));
    if (!confirmed.ok) return json(confirmed, 400, headers);
    return json({ ok: true, confirmed: true }, 200, headers);
  }
