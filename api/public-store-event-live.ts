...
async function readByRole(db: Db, token: string, role: string, headers: Record<string, string>) {
  ...
  const clock = await applyStoreTrialClock(db, data, STORE_EVENT_LIVE_TABLE);
  if (clock.expired) {
    return json(
      {
        ok: true,
  ...
  const retention = await applyStoreLiveRetentionClock(db, 'event', data);
...
async function addBlessing(db: Db, body: Record<string, unknown>, headers: Record<string, string>) {
  ...
  if (eventBlessingExtraBlocked(extra)) {
...
async function saveHost(db: Db, body: Record<string, unknown>, headers: Record<string, string>) {
  ...
  const retentionGuard = await applyStoreLiveRetentionClock(db, 'event', data);
