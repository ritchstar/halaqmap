...
assert.ok(produceLanding.includes('STORE_PRODUCE_LIVE.trialCtaAr'));
assert.ok(produceLanding.includes('STORE_PRODUCE_LIVE.extensionTitleAr'));
...
assert.equal(STORE_PRODUCE_LIVE_PACKS[0].priceLineAr, '1350 ر.س');
assert.equal(STORE_PRODUCE_LIVE_PACKS[1].priceLineAr, '2500 ر.س');
...
assert.match(STORE_PRODUCE_LIVE.locateMeAr, /تحديد موقعي/);
assert.match(STORE_PRODUCE_LIVE.confirmPlaceAr, /تأكيد الموقع/);
assert.doesNotMatch(STORE_PRODUCE_LIVE.solutionBodyAr, /خلال ثوان/);
assert.match(produceLanding, /product=produce/);
assert.match(readFileSync(join(root, 'src/components/store/StoreProduceShop.tsx'), 'utf8'), /useState\(false\)/);
