...
const CLUSTERS = [
  ...
  },
  {
    ...
    note: 'عبارة «حجز» شائعة في البحث. عبر حلاق ماب تبدأ الاستعلام ثم تتواصل مباشرة مع الصالون لترتيب موعدك.',
    phrases: [
...
function shell({ title, description, canonical, h1, bodyInner, jsonLd, keywords, footerNote }) {
  const footer =
    footerNote ||
    '© حلاق ماب — فزعة استعلام لحظي عن أقرب حلاق وصالون قريب في أوقات المناسبات.';
  return `<!DOCTYPE html>
...
${brandPageTypeCss('linear-gradient(180deg,#120a06,#0a1f33 40%,#061223 100%)')}
...
function renderEidPage() {
  ...
  return shell({
    ...
    bodyInner: `
      ...
      <p class="lead">زحام عيد الأضحى <strong>نهاري وملح ومقترن بعبادة وتوقيت دقيق</strong>: ذبح، سلخ، أضحية، ثم حاجة فورية لإزالة الشعر أو التقصير للتحلل. هذه فزعة من <strong>حلاق ماب</strong> لتسهيل بدء الاستعلام بفلتر «مفتوح الآن» وأقرب نطاق بعد الأضحية.</p>
...
function renderRamadanPage() {
  ...
  const faqs = [
    ...
    },
    {
      ...
      a: 'دور فزعة حلاق ماب تقني: مساعدتك على بدء استعلام عن حلاق قريب أو مفتوح لحظة حاجتك.',
    },
  ];
  return renderOccasionPage({
    ...
    footerNote:
      '© حلاق ماب — فزعة استعلام عن حلاق مفتوح في رمضان والمساء وبعد التراويح.',
    faqs,
    leadHtml: `
      <p class="lead">في رمضان تتأخّر ذروة الحلاقة إلى <strong>المساء وبعد التراويح وقرب السحور</strong> — بعكس الصباح المعتاد. هذه فزعة من <strong>حلاق ماب</strong> لتسهيل بدء الاستعلام حين تحتاج حلاقاً مفتوحاً في ذلك التوقيت — ابدأ الاستعلام لحظة حاجتك.</p>
    `,
    sectionsHtml: `
      <section class="card warn">
...
function renderFridayPage() {
  ...
  const faqs = [
    ...
    },
    {
      ...
      a: 'كثير يبحث ليلة الخميس أو صباح الجمعة قبل الصلاة. استخدم «مفتوح الآن» لحظة حاجتك، أو رتّب مع الصالون مسبقاً عبر التواصل المباشر.',
    },
    {
  ...
  return renderOccasionPage({
    ...
    footerNote: '© حلاق ماب — فزعة استعلام لحظي عن أقرب حلاق قبل الجمعة وليلة الخميس.',
    faqs,
    leadHtml: `
