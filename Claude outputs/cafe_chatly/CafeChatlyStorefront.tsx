...
export function CafeChatlyStorefront({
  ...
  function submit() {
    ...
    showNotice(`وصلت تذكرة الكاشير رقم ${ticketNo}.`);
  }
  ...
  return (
    <main dir="rtl" className="cafe-chatly-root min-h-dvh bg-[#f4f1e8] pb-32 text-[#2a1e14]">
      <div className="mx-auto w-full max-w-[1240px] px-4 sm:px-8 lg:px-10">
        <header className="flex items-center justify-between border-b border-[#e4ddcd] py-5">
          <div className="flex min-w-0 items-center gap-3">
            <div className="flex size-10 shrink-0 items-center justify-center rounded-full border-[#e6d2b5] bg-[#fffdf5] text-[#a8703a]">
              <StoreShopLogoMark src={state.host.logoSrc} className="size-8" />
              {!state.host.logoSrc ? <CafeKafenaMark size="sm" /> : null}
            </div>
        ...
        {state.host.flashAr.trim() ? (
          <p className="mt-4 overflow-hidden rounded-2xl border border-[#e6d2b5] bg-[#f5e8d8] px-4 py-2 text-sm font-bold text-[#a8703a]">
            {state.host.flashAr}
          </p>
        ) : null}
        ...
        <section className="grid gap-8 border-b border-[#e4ddcd] py-9 lg:grid-cols-[1fr_0.86fr] lg:items-center lg:gap-16">
          <div>
            <div className="mb-4 flex items-center gap-2 text-xs font-bold text-[#c48a4a]">
              <span className="h-px w-8 bg-[#c48a4a]" />
            ...
            <p className="mt-5 max-w-lg text-base leading-8 text-[#7a6e5c]">{state.host.blurbAr}</p>
            {state.host.customFields.some((line) => line.trim()) ? (
              <ul className="mt-4 space-y-1 text-sm leading-7 text-[#867a67]">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              {today ? (
                <div className="inline-flex items-center gap-2 rounded-full border border-[#e6d2b5] bg-[#fffdf5] px-4 py-3 text-xs font-bold text-[#736750]">
                  <span className="size-2 rounded-full bg-[#c48a4a]" />
                  {STORE_CAFE_LIVE.todayTitleAr}: {today.nameAr}
                </div>
              ) : null}
            </div>
            <div className="mt-8 flex flex-wrap gap-5 text-xs font-bold text-[#867a67]">
              {hoursLine ? (
          ...
          <div className="relative">
            <div className="absolute -left-8 -top-8 size-24 rounded-full border border-dashed border-[#e6d2b5]" />
            <div className="relative overflow-hidden rounded-[2rem] border-[9px] border-[#fffdf5] bg-[#f0e4d0] shadow-[0_22px_60px_rgba(196,138,74,0.15)]">
              <img
                src={heroSrc}
                alt=""
        ...
        {featured.length ? (
          <section className="border-b border-[#e4ddcd] py-9">
            <p className="text-xs font-bold tracking-[0.16em] text-[#c48a4a]">{STORE_CAFE_LIVE.featuredTitleAr}</p>
            <h2 className="mt-2 text-2xl font-black tracking-[-0.05em] text-[#2a1e14]">{STORE_CAFE_LIVE.featuredTitleAr}</h2>
        ...
        {today ? (
          <section className="border-b border-[#e4ddcd] py-9">
            <p className="text-xs font-bold tracking-[0.16em] text-[#a8703a]">{STORE_CAFE_LIVE.todayTitleAr}</p>
            <div className="mt-4 overflow-hidden rounded-2xl border border-[#e6d2b5] bg-[#fffdf5]">
              {today.photoSrc ? (
        ...
        <section className="sticky top-0 z-20 -mx-4 border-b border-[#e4ddcd] bg-[#f4f1e8]/95 px-4 py-3 backdrop-blur-sm sm:-mx-8 sm:px-8 lg:-mx-10 lg:px-10">
          <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center">
            <div className="relative min-w-0 flex-1">
              <Search size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#88796a]" />
              <Input
                value={search}
                onChange={(event) => setSearch(event.target.value)}
                placeholder={visible.length >= 8 ? 'ابحث في القائمة…' : ''}
                className="h-11 border-[#ddd0b8] bg-[#fffdf5] pr-10 text-sm text-[#2a1e14] placeholder:text-[#aca090]"
              />
            </div>
        ...
        <section id="shelf" className="py-9">
          <div className="flex items-end justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-black tracking-[-0.05em] text-[#2a1e14]">اختَر ما يعجبك</h2>
            </div>
            <span className="hidden items-center gap-2 text-xs font-bold text-[#867a67] sm:flex">
          ...
          {!filtered.length ? (
            <div className="mt-6 rounded-2xl border border-dashed border-[#e8dfd0] bg-[#fffdf5] p-10 text-center">
              <Coffee size={24} className="mx-auto text-[#a8703a]" />
              <p className="mt-3 font-black text-[#2a1e14]">ما لقينا هذا الصنف</p>
              <p className="mt-1 text-sm text-[#88796a]">جرّب كلمة ثانية أو ارجع لكل القائمة.</p>
            </div>
          ) : null}
        </section>

        <section className="grid gap-3 border-t border-[#e4ddcd] py-8 sm:grid-cols-3">
          ...
          <Feature icon={Store} title="متجر مستقل" copy="طلبك يذهب مباشرة إلى هذا المقهى." />
        </section>

        <section className="rounded-2xl border border-[#ddd0b8] bg-[#fffdf5] p-5">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
            <div>
              <p className="text-xs font-bold tracking-[0.14em] text-[#c48a4a]">متجر مستقل</p>
              <h3 className="mt-1 text-lg font-black text-[#2a1e14]">أنت تشتري من بائع واحد في حيك</h3>
              <p className="mt-1 text-sm text-[#867a67]">ليست سوقاً مشتركة؛ طلبك يذهب مباشرة إلى {state.host.shopName}.</p>
            </div>
        ...
        <footer className="flex flex-col gap-3 border-t border-[#e4ddcd] py-7 text-sm text-[#867a67] sm:flex-row sm:items-center sm:justify-between">
          <p>كافينا1 — من مقهى الحي إلى فنجانك.</p>
        ...
        <div className="fixed bottom-4 right-4 left-4 z-30 mx-auto flex max-w-[800px] items-center justify-between gap-4 rounded-2xl bg-[#2a1e14] px-4 py-3 text-white shadow-[0_15px_35px_rgba(42,30,20,0.28)] sm:right-8 sm:left-8 sm:px-5">
          <div className="flex min-w-0 items-center gap-3">
            <span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-[#c48a4a]">
              <CafeKafenaMark size="sm" inverse />
            </span>
            <div className="min-w-0">
              <p className="truncate text-sm font-black">
                {cartItemCount} {cartItemCount === 1 ? 'صنف' : 'أصناف'} في السلة
              </p>
              <p className="text-xs text-[#e4d4bd]">
          ...
          <Button
            ...
            className="h-10 shrink-0 rounded-xl border-0 bg-[#f5e8d8] px-4 text-xs font-black text-[#4a3418] shadow-none hover:bg-white"
          >
            إتمام الطلب
            <ArrowLeft size={15} />
          </Button>
      ...
      <Dialog open={cartOpen} onOpenChange={setCartOpen}>
        <DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto border-[#ddd0b8] bg-[#fffdf5] p-0 text-right" dir="rtl">
          <div className="p-5 sm:p-6">
            <DialogHeader>
              <div className="flex items-center justify-between gap-3">
                <div>
                  <DialogTitle className="text-2xl font-black text-[#2a1e14]">مراجعة السلة</DialogTitle>
                  <DialogDescription className="mt-2 text-sm leading-6 text-[#867a67]">
                    الإجمالي يُحسب عند الكاشير عند التسليم.
                  </DialogDescription>
                </div>
                <span className="rounded-full bg-[#f5e8d8] px-3 py-1 text-xs font-black text-[#c48a4a]">
                  {cartItemCount} أصناف
                </span>
              </div>
            </DialogHeader>
            ...
            <div className="mt-6 rounded-xl bg-[#f5e8d8] p-4">
      ...
      <Dialog open={serviceOpen} onOpenChange={setServiceOpen}>
        <DialogContent className="max-w-xl border-[#ddd0b8] bg-[#fffdf5] text-right" dir="rtl">
          <DialogHeader>
            <DialogTitle className="text-2xl font-black text-[#2a1e14]">كيف توصلك طلبيتك؟</DialogTitle>
            <DialogDescription className="mt-2 text-sm leading-6 text-[#867a67]">
            ...
            {services.map(({ id, label, icon: Icon, copy }) => (
              <Button
                ...
                className={cn(
                  ...
                  service === id
                    ? 'border-[#c48a4a] bg-[#f5e8d8]'
                    : 'border-[#e8dfd0] bg-white hover:bg-[#faf3e6]',
                )}
              >
          ...
          {service === 'pickup' && state.host.pickupPlaceVisible && state.host.pickupMapsUrl ? (
            <div className="mt-5 rounded-xl border border-[#ddd0b8] bg-[#f5e8d8] p-4">
              <div className="flex items-center gap-2 text-sm font-black text-[#a8703a]">
                <MapPin size={16} />
                موقع الاستلام
              </div>
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-2 inline-block text-sm font-bold leading-6 text-[#867a67] underline"
              >
                {STORE_CAFE_LIVE.pickupPlaceOpenAr}
              </a>
            </div>
          ) : null}
          <Button
      ...
      <Dialog open={checkoutOpen} onOpenChange={setCheckoutOpen}>
        <DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto border-[#ddd0b8] bg-[#fffdf5] text-right" dir="rtl">
          <DialogHeader>
          ...
          >
            <label className="block text-xs font-bold text-[#736750]">
              {STORE_CAFE_LIVE.buyerNameLabelAr}
              <Input
                required
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="mt-2 h-11 border-[#ddd0b8] bg-white"
                maxLength={40}
              />
            </label>
            <label className="block text-xs font-bold text-[#736750]">
              ...
              <>
                <label className="block text-xs font-bold text-[#736750]">
                  ...
                  <Input
                    ...
                    className="mt-2 h-11 border-[#ddd0b8] bg-white"
                    maxLength={160}
                    placeholder="الحي، الشارع، رقم المبنى أو أقرب علامة"
                  />
                </label>
            ...
            <label className="flex items-start gap-2 text-sm leading-7 text-[#867a67]">
      ...
      {notice ? (
        <div className="fixed bottom-24 right-4 z-50 flex items-center gap-2 rounded-full bg-[#2a1e14] px-5 py-3 text-sm font-bold text-white shadow-xl sm:right-8">
          <Check size={16} className="text-[#f5e8d8]" />
          {notice}
        </div>
      ) : null}
    </main>
...
function ProductCard({
  ...
  return (
    <article className="group overflow-hidden rounded-2xl border border-[#e8dfd0] bg-[#fffdf5] shadow-[0_10px_25px_rgba(42,30,20,0.05)]">
      <ProductVisual item={item} />
      <div className="p-4">
        <div className="flex items-start justify-between gap-2">
          <div>
            <h3 className="text-base font-black text-[#2a1e14]">{item.nameAr}</h3>
            {item.category ? <p className="mt-1 text-xs font-bold text-[#a89c86]">{item.category}</p> : null}
          </div>
        ...
        <div className="mt-4 flex items-center justify-between">
          <span
            className={cn(
              ...
              item.featured ? 'text-[#c48a4a]' : 'text-[#a89c86]',
            )}
          >
          ...
          {quantity ? (
            ...
            <Button
              type="button"
              onClick={() => onChange(1)}
              className="h-9 rounded-lg border-0 bg-[#f5e8d8] px-3 text-xs font-black text-[#a8703a] shadow-none hover:bg-[#ecd8ba]"
            >
              إضافة سريعة
              <Plus size={14} />
            </Button>
        ...
        <p className="mt-2 text-[10px] text-[#a89c86]">{priceLine}</p>
      </div>
    </article>
  );
}
...
function ProductVisual({ item, small = false }: { item: ShelfRow; small?: boolean }) {
  ...
  return (
    <div className={cn('relative overflow-hidden bg-[#f0e4d0]', small ? 'size-16 shrink-0 rounded-lg' : '')}>
      <img
        src={src}
        alt={item.nameAr}
        className={cn(
...
function QtyControls({
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg bg-[#f5e8d8] px-2 py-1">
      <Button type="button" onClick={onMinus} className="size-7 rounded-md bg-transparent p-0 text-[#a8703a] shadow-none hover:bg-white">
        <Minus size={14} />
      </Button>
      <span className="w-4 text-center text-sm font-black">{value}</span>
      <Button type="button" onClick={onPlus} className="size-7 rounded-md bg-transparent p-0 text-[#a8703a] shadow-none hover:bg-white">
        <Plus size={14} />
      </Button>
    </div>
  );
...
function Feature({ icon: Icon, title, copy }: { icon: typeof Truck; title: string; copy: string }) {
  return (
    <div className="flex gap-3 rounded-2xl border border-[#e8dfd0] bg-[#fffdf5] p-4">
      <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-[#f5e8d8] text-[#c48a4a]">
        <Icon size={17} />
      </span>
      <div>
        <p className="text-sm font-black text-[#2a1e14]">{title}</p>
        <p className="mt-1 text-xs leading-5 text-[#a89c86]">{copy}</p>
      </div>
    </div>
  );
}
