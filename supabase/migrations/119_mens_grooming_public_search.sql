...
-- تغيير أعمدة الإرجاع يتطلب حذف الدالة أولاً (لا يكفي CREATE OR REPLACE)
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'row', jsonb_build_object(
      ...
      'mens_grooming_center', COALESCE(v_barber.mens_grooming_center, false),
      'grooming_center_banner_lines', COALESCE(v_barber.grooming_center_banner_lines, '[]'::jsonb),
...
COMMENT ON FUNCTION public.get_public_showcase_fallback IS
  'يعيد حلاق المعاينة + home_service + groom_prep + children_specialist + mens_grooming_center عند fallback.';
