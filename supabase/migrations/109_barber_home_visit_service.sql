-- =====================================================
-- زيارة منزلية (ذهبي/ماسي): إعلان + بحث جغرافي
-- =====================================================

ALTER TABLE public.barbers
  ADD COLUMN IF NOT EXISTS home_service_offered boolean NOT NULL DEFAULT false;

ALTER TABLE public.barbers
  ADD COLUMN IF NOT EXISTS home_service_price_sar numeric;

ALTER TABLE public.barbers
  ADD COLUMN IF NOT EXISTS home_service_radius_km integer;

ALTER TABLE public.barbers
  ADD COLUMN IF NOT EXISTS home_service_public_visible boolean NOT NULL DEFAULT true;

ALTER TABLE public.barbers
  ADD COLUMN IF NOT EXISTS home_service_customer_note text;

COMMENT ON COLUMN public.barbers.home_service_offered IS
  'الحلاق يعلن توفير زيارة منزلية — ذهبي/ماسي فقط في لوحة التحكم.';

COMMENT ON COLUMN public.barbers.home_service_price_sar IS
  'سعر إرشادي معروض للعميل (ريال) عند تفعيل الزيارة المنزلية.';

COMMENT ON COLUMN public.barbers.home_service_radius_km IS
  'نطاق تغطية تقريبي بالكيلومتر — إرشادي للعميل.';

COMMENT ON COLUMN public.barbers.home_service_public_visible IS
  'إخفاء مؤقت عن بطاقة العرض دون حذف الإعدادات.';

COMMENT ON COLUMN public.barbers.home_service_customer_note IS
  'ملاحظة للعميل (شروط، تنسيق مسبق، …) — لا عنوان منزل.';
...
COMMENT ON FUNCTION public.search_barbers_nearby IS
  'بحث جغرافي + حقن المعاينة + children_specialist + home_service للزيارة المنزلية.';
