...
-- =====================================================
-- قطاع الإدراج: mens_barber (افتراضي) | coiffeur_women
-- البحث الرجالي لا يرى كوافير ماب. استعلام كوافير له عرض مستقل.
-- DROP VIEW CASCADE ضروري لأن SELECT b.* يجمّد الأعمدة عند إنشاء العرض.
-- يُعاد إنشاء search_barbers_nearby بنفس توقيع 119 + شرط الاستحقاق من 125.
-- =====================================================

ALTER TABLE public.barbers
  ADD COLUMN IF NOT EXISTS listing_sector text NOT NULL DEFAULT 'mens_barber';

ALTER TABLE public.barbers
  DROP CONSTRAINT IF EXISTS barbers_listing_sector_check;

ALTER TABLE public.barbers
  ADD CONSTRAINT barbers_listing_sector_check
  CHECK (listing_sector IN ('mens_barber', 'coiffeur_women'));

COMMENT ON COLUMN public.barbers.listing_sector IS
  'قطاع الإدراج: mens_barber لبحث حلاق ماب، coiffeur_women لكوافير ماب فقط.';

CREATE INDEX IF NOT EXISTS barbers_listing_sector_idx
  ON public.barbers (listing_sector);
...
WHERE b.is_active = true
  ...
  AND COALESCE(b.listing_sector, 'mens_barber') <> 'coiffeur_women'
  AND (
...
COMMENT ON VIEW public.barbers_public_directory IS
  'حلاق يظهر على الخريطة/البحث العام الرجالي: نشط + إدراج ساري — يستثني coiffeur_women وعرض المنصة التوضيحي.';
...
CREATE OR REPLACE VIEW public.coiffeur_public_directory AS
SELECT
  b.*,
  true AS has_active_subscription
FROM public.barbers b
WHERE b.is_active = true
  AND COALESCE(b.is_showcase_preview, false) IS NOT TRUE
  AND b.listing_sector = 'coiffeur_women'
  AND (
    public.barber_has_active_listing(b.id)
    OR EXISTS (
      SELECT 1
...
COMMENT ON VIEW public.coiffeur_public_directory IS
  'مشغل نسائي يظهر في استعلام كوافير ماب فقط — لا يُستخدم في search_barbers_nearby.';

GRANT SELECT ON public.coiffeur_public_directory TO anon, authenticated, service_role;

ALTER VIEW public.coiffeur_public_directory SET (security_invoker = true);
...
RETURNS TABLE (
  ...
  mens_grooming_center boolean,
  grooming_center_banner_lines jsonb,
...
base AS (
  SELECT
    ...
    COALESCE(b.mens_grooming_center, false) AS mens_grooming_center,
    COALESCE(b.grooming_center_banner_lines, '[]'::jsonb) AS grooming_center_banner_lines,
...
real_results AS (
  SELECT
    ...
    b.groom_prep_customer_note,
    b.mens_grooming_center,
    b.grooming_center_banner_lines,
...
showcase_row AS (
  SELECT
    ...
    COALESCE(sb.mens_grooming_center, false) AS mens_grooming_center,
    COALESCE(sb.grooming_center_banner_lines, '[]'::jsonb) AS grooming_center_banner_lines,
    true AS has_active_subscription,
  ...
  WHERE COALESCE(sb.is_showcase_preview, false) IS TRUE
    ...
    AND COALESCE(sb.listing_sector, 'mens_barber') <> 'coiffeur_women'
...
COMMENT ON FUNCTION public.search_barbers_nearby IS
  'بحث جغرافي رجالي من barbers_public_directory — يستثني قطاع coiffeur_women.';
