...
/** دعوة زواج تفاعلية — منتج مستقل عن بطاقة المناسبة وعن رخصة النفاذ. */
function isWeddingLiveMeta(meta: Record<string, unknown> | null | undefined): boolean {
  if (!meta || typeof meta !== "object") return false;
  const pt = String(meta.product_type ?? meta.productType ?? meta.product ?? "").trim().toLowerCase();
...
/** مطعمنا1 — اشتراك صاحب المطعم، مستقل عن تموينات الحي والقاعات ورخصة النفاذ. */
function isRestaurantLiveMeta(meta: Record<string, unknown> | null | undefined): boolean {
  if (!meta || typeof meta !== "object") return false;
  const pt = String(meta.product_type ?? meta.productType ?? meta.product ?? "").trim().toLowerCase();
...
function isHalanaLiveMeta(meta: Record<string, unknown> | null | undefined): boolean {
  if (!meta || typeof meta !== "object") return false;
  const pt = String(meta.product_type ?? meta.productType ?? meta.product ?? "").trim().toLowerCase();
...
Deno.serve(async (req) => {
  ...
  if (isWeddingLiveMeta(meta)) {
    const token = String(meta.store_wedding_token ?? meta.storeWeddingToken ?? "").trim();
    const amountOk = amount === 89900;
    let activated = false;
    if (successStatus && token && amountOk) {
      const { error: weddingErr } = await supabase
        .from("store_wedding_live_orders")
        .update({
          status: "live",
          moyasar_payment_id: paymentId,
          last_public_change_at: new Date().toISOString(),
  ...
  if (isHalanaLiveMeta(meta)) {
    const token = String(meta.store_halana_token ?? meta.storeHalanaToken ?? "").trim();
    const days = amount === 178800 ? 360 : amount === 89400 ? 180 : 0;
    let activated = false;
    if (successStatus && token && days) {
      const expiresAt = new Date(Date.now() + days * 24 * 60 * 60 * 1000).toISOString();
      const { data: halanaRow, error: halanaErr } = await supabase
        .from("store_halana_copies")
        .update({
          status: "live",
          moyasar_payment_id: paymentId,
          expires_at: expiresAt,
        ...
        .eq("shop_token", token)
        .in("status", ["pending_payment", "pending_renewal", "expired", "issued"])
        .select("id")
        .maybeSingle();
      activated = !halanaErr;
      if (halanaRow?.id) {
        await supabase
          .from("store_product_trials")
          .update({ status: "converted", updated_at: new Date().toISOString() })
          .eq("order_id", halanaRow.id)
          .in("status", ["issued", "activated", "expired"]);
      }
    }
    const credited = successStatus && days
      ? await creditStoreAffiliateLedger(supabase, "store_halana_live", amount, paymentId, meta)
      : false;
    return jsonResponse(
      {
        ok: true,
